...
function generateStockStory(data, rating) {
  ...
  const pct = (v) => v != null && !isNaN(v) ? (v * 100).toFixed(2) + '%' : 'N/D';

  let md = '';
  md += `# Stock Story: ${ticker}\n`;
  ...
  md += `| IPO | ${profile?.ipo || 'N/D'} |\n\n`;

  // Price
  md += `## 2. Cotação & Performance\n\n`;
  md += `| Métrica | Valor |\n|---|---|\n`;
  md += `| Preço Atual | ${formatCurrency(quote?.c, profile?.currency)} |\n`;
  md += `| Variação Dia | ${nd(quote?.dp)}% |\n`;
  md += `| Abertura | ${formatCurrency(quote?.o, profile?.currency)} |\n`;
  md += `| 52W High | ${formatCurrency(fund.fiftyTwoHigh, profile?.currency)} |\n`;
  md += `| 52W Low | ${formatCurrency(fund.fiftyTwoLow, profile?.currency)} |\n`;
  ...
  // Credit
  md += `## 4. Saúde Financeira (Credit Lens)\n\n`;
  ...
  md += `| Quick Ratio | ${nd(credit.quickRatio)} | — |\n`;
  ...
  if (recommendations && recommendations.length > 0) {
    ...
    if (priceTarget?.targetMean) {
      ...
      md += `**Target Price Médio:** ${formatCurrency(priceTarget.targetMean, profile?.currency)} (${upside >= 0 ? '+' : ''}${upside.toFixed(1)}% upside)\n\n`;
    }
  } else {
  ...
  // Rating
  md += `## 6. Rating Proprietário\n\n`;
  if (rating) {
    md += `**Score Total: ${rating.total}/100** (${rating.total >= 70 ? 'Forte' : rating.total >= 50 ? 'Médio' : 'Fraco'})\n\n`;
  ...
  md += `---\n`;
  md += `*Relatório gerado automaticamente pelo Aegir·Intel em ${date}. Dados: ${isBR ? 'brapi.dev' : 'Finnhub'}. Não constitui recomendação de investimento.*\n`;
