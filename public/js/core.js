...
function _renderInternal() {
  ...
  if (state.detail) {
    ...
    switch (state.view) {
      ...
      case 'am_risk':       content = renderPlaceholder('Risk Analysis', 'Análise de risco da gestora e dos fundos. VaR, stress test, concentração, liquidez. Em desenvolvimento.'); break;
      case 'am_comite':     content = renderComite(); break;
      case 'am_product':    content = renderPlaceholder('Product Analysis', 'Viabilidade de novos fundos de investimento. Simulação de PL, taxas, benchmark e público-alvo. Em desenvolvimento.'); break;
      case 'am_assets_analysis': content = renderPlaceholder('Assets Analysis', 'Análise e rating de ativos individuais para decisão de alocação. Em desenvolvimento.'); break;
      case 'am_credit_port': content = renderPlaceholder('Credit Portfolio', 'Visualização consolidada do portfólio de crédito privado da gestora. Duration, indexadores, concentração. Em desenvolvimento.'); break;
      case 'am_equity_port': content = renderPlaceholder('Equity Portfolio', 'Visualização consolidada do portfólio de equity da gestora. Setores, fatores, performance. Em desenvolvimento.'); break;
