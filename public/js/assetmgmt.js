...
function renderPanoramaChart(title, series, unit, color, showZeroLine) {
  ...
  return h('div', { class: 'card', style: { padding: '14px' } }, [
    ...
    h('svg', { viewBox: `0 0 ${w} ${ht}`, style: { width: '100%' }, html: svg }),
  ]);
}
...
function renderCarteiraXMLDetail(parsed, fund) {
  ...
  return h('div', {}, [
    ...
    // Títulos Públicos
    positions.titpublico.length > 0 && h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead' }, `Títulos Públicos (${positions.titpublico.length})`),
      h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } }, [
        h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, [
          h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
          ...
          h('tbody', {},
            positions.titpublico.sort((a, b) => (b.valorfindisp || 0) - (a.valorfindisp || 0)).map(p =>
    ...
    // Imóveis
    positions.imoveis.length > 0 && h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead' }, `Imóveis (${positions.imoveis.length})`),
      ...positions.imoveis.map(im => {
...
function computeFundRisk(parsed) {
  ...
  const allPos = [...positions.acoes, ...positions.titprivado, ...positions.cotas];
  const now = new Date();
  ...
  // PDD by risk level
  const pddByRisco = {};
  for (const p of pddByPosition) {
    const r = p.risco;
    if (!pddByRisco[r]) pddByRisco[r] = { count: 0, value: 0, pdd: 0 };
    pddByRisco[r].count++;
    pddByRisco[r].value += p.value;
    pddByRisco[r].pdd += p.pddValue;
  }

  // ===== 3. LIQUIDITY =====
  const cashRatio = (stats.totalCaixa / pl) * 100;
  ...
  for (const p of creditPositions) {
    ...
    const venc = new Date(p.dtvencimento);
    const years = (venc.getTime() - now.getTime()) / (365.25 * 86400000);
    const bucket = years <= 1 ? '0-1Y' : years <= 3 ? '1-3Y' : years <= 5 ? '3-5Y' : years <= 10 ? '5-10Y' : '10Y+';
    maturityBuckets[bucket] += val;
    maturityBucketCounts[bucket]++;
  }

  // Weighted average maturity
  let wamSum = 0, wamWeight = 0;
  for (const p of creditPositions) {
    if (p.dtvencimento && p.valorfindisp) {
      const years = (new Date(p.dtvencimento).getTime() - now.getTime()) / (365.25 * 86400000);
      if (years > 0) { wamSum += years * p.valorfindisp; wamWeight += p.valorfindisp; }
    }
  }
  const wam = wamWeight > 0 ? wamSum / wamWeight : null;

  // ===== 4. INDEXADOR EXPOSURE =====
  const idxExposure = {};
  for (const p of creditPositions) {
    const idx = p.indexador || 'Outros';
    if (!idxExposure[idx]) idxExposure[idx] = { value: 0, count: 0 };
    idxExposure[idx].value += p.valorfindisp || 0;
    idxExposure[idx].count++;
  }

  // ===== 5. DURATION SENSITIVITY =====
  // Estimate: for each +100bps in rates, duration-based approx impact
  let durationSum = 0, durWeight = 0;
  for (const p of creditPositions) {
    if (p.dtvencimento && p.valorfindisp) {
      const years = (new Date(p.dtvencimento).getTime() - now.getTime()) / (365.25 * 86400000);
      if (years > 0) { durationSum += years * p.valorfindisp; durWeight += p.valorfindisp; }
    }
  }
  const avgDuration = durWeight > 0 ? durationSum / durWeight : 0;
  // Modified duration approx: impact = -duration * Δy * value
  const impact100bps = avgDuration * 0.01 * totalCreditValue; // value lost per +100bps
  const impact200bps = avgDuration * 0.02 * totalCreditValue;

  // ===== 6. STRESS SCENARIOS =====
  const stressScenarios = [
    {
      name: 'SELIC +200 bps',
      desc: 'Elevação de 200bps na curva de juros',
      impactValue: -impact200bps,
      impactPct: pl > 0 ? (-impact200bps / pl) * 100 : 0,
    },
    {
      name: 'Default Top 1',
      desc: `Inadimplência total do maior ativo (${maxSingle.name})`,
      impactValue: -(maxSingle.value || 0),
      impactPct: pl > 0 ? (-(maxSingle.value || 0) / pl) * 100 : 0,
    },
    {
      name: 'PDD +50%',
      desc: 'Aumento de 50% nas provisões de crédito',
      impactValue: -(totalPDD * 0.5),
      impactPct: pl > 0 ? (-(totalPDD * 0.5) / pl) * 100 : 0,
    },
    {
      name: 'Resgate 20% PL',
      desc: 'Necessidade de liquidar 20% do patrimônio',
      impactValue: -(pl * 0.2),
      impactPct: -20,
      liquidityNote: cashRatio >= 20 ? 'Caixa suficiente' : `Déficit de ${formatBRL(pl * 0.2 - stats.totalCaixa)}`,
    },
  ];

  // ===== 7. VaR PARAMÉTRICO (simplificado) =====
  // Usando duration como proxy de volatilidade
  // VaR 95% ≈ 1.65 * σ_diário * √(horizonte) * valor
  // σ estimada = duration * σ_taxa (assumindo ~15bps/dia para crédito BR)
  const sigmaRate = 0.0015; // 15bps daily
  const horizon = 21; // 1 mês
  const portfolioSigma = avgDuration * sigmaRate * Math.sqrt(horizon);
  const var95 = 1.645 * portfolioSigma * totalCreditValue;
  const var99 = 2.326 * portfolioSigma * totalCreditValue;

  return {
    fund, pl,
    concentration: { hhi, top5, top5Pct, top10Pct, maxSingle, totalPositions: allPos.length },
    credit: { totalCreditValue, totalPDD, pddCoveragePct, pddByRisco, pddByPosition, creditCount: creditPositions.length },
    liquidity: { cashRatio, totalCaixa: stats.totalCaixa, maturityBuckets, maturityBucketCounts, wam },
    indexador: idxExposure,
    duration: { avgDuration, impact100bps, impact200bps },
    stress: stressScenarios,
    var: { var95, var99, portfolioSigma, horizon },
  };
}
      ...
      : h('div', {}, [
          ...
          ...funds.map(fund => {
            ...
            return h('div', {
              ...
              h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 100px 100px 100px 100px 100px', gap: '12px', alignItems: 'center' } }, [
                h('div', {}, [
                  h('div', { style: { fontFamily: 'Fraunces, serif', fontSize: '15px' } }, fund.name),
                ...
                renderRiskMiniKPI('PDD %', risk?.credit?.pddCoveragePct != null ? `${risk.credit.pddCoveragePct.toFixed(1)}%` : '—'),
                renderRiskMiniKPI('Duration', risk?.duration?.avgDuration != null ? `${risk.duration.avgDuration.toFixed(1)}a` : '—'),
                renderRiskMiniKPI('VaR 95%', risk?.var?.var95 != null ? formatBRL(risk.var.var95) : '—'),
              ]),
            ]);
          }),
        ]),
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' } }, [
      h('div', {}, [
        ...
        h('h1', { class: 'page-title' }, fund.name),
        h('div', { class: 'mono', style: { fontSize: '11px', color: 'var(--text-faint)' } },
          `${fund.classification?.toUpperCase() || '?'} · Carteira: ${carteira?.fund?.dtposicao || '—'} · PL: ${risk ? formatBRL(risk.pl) : '—'}`),
      ]),
      h('button', { class: 'btn-secondary', onClick: () => exportRiskReport(fund, risk) }, '↓ Relatório .md'),
    ]),
    ...
    !risk
      ...
      : h('div', {}, [
          ...
          h('div', { class: 'sec-tab-row', style: { marginBottom: '20px' } },
            [['overview', 'Resumo'], ['concentration', 'Concentração'], ['credit', 'Crédito & PDD'], ['liquidity', 'Liquidez'], ['stress', 'Stress Test']].map(([k, l]) =>
          ...
          subTab === 'stress' ? renderRiskStress(risk) : null,
        ]),
  ]);
}
...
function renderRiskOverview(risk) {
  ...
  return h('div', {}, [
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '28px' } }, [
      ...
      renderRiskKPI('Top 5 Concentração', `${risk.concentration.top5Pct.toFixed(1)}%`, `${risk.concentration.totalPositions} posições`, riskColor(risk.concentration.top5Pct, [60, 80])),
      renderRiskKPI('PDD / Crédito', `${risk.credit.pddCoveragePct.toFixed(2)}%`, `${formatBRL(risk.credit.totalPDD)} de ${formatBRL(risk.credit.totalCreditValue)}`, riskColor(risk.credit.pddCoveragePct, [5, 15])),
      renderRiskKPI('Duration Média', risk.duration.avgDuration > 0 ? `${risk.duration.avgDuration.toFixed(2)} anos` : 'N/A', risk.duration.avgDuration > 5 ? 'Longa' : risk.duration.avgDuration > 2 ? 'Média' : 'Curta', riskColor(risk.duration.avgDuration, [3, 7])),
      renderRiskKPI('Caixa / PL', `${risk.liquidity.cashRatio.toFixed(2)}%`, formatBRL(risk.liquidity.totalCaixa), riskColor(100 - risk.liquidity.cashRatio, [95, 99])),
      renderRiskKPI('VaR 95% (21d)', formatBRL(risk.var.var95), `${(risk.var.var95 / risk.pl * 100).toFixed(2)}% do PL`, riskColor(risk.var.var95 / risk.pl * 100, [3, 8])),
      renderRiskKPI('VaR 99% (21d)', formatBRL(risk.var.var99), `${(risk.var.var99 / risk.pl * 100).toFixed(2)}% do PL`, riskColor(risk.var.var99 / risk.pl * 100, [5, 12])),
      renderRiskKPI('Maior Posição', risk.concentration.maxSingle.name, `${risk.concentration.maxSingle.pct.toFixed(1)}% do PL`, riskColor(risk.concentration.maxSingle.pct, [15, 25])),
    ]),
    ...
    h('div', { class: 'card', style: { padding: 0, overflow: 'hidden' } },
      [
        ...
        { category: 'Crédito', level: risk.credit.pddCoveragePct < 5 ? 'Baixo' : risk.credit.pddCoveragePct < 15 ? 'Médio' : 'Alto', detail: `PDD: ${risk.credit.pddCoveragePct.toFixed(2)}% · ${risk.credit.creditCount} ativos` },
        { category: 'Liquidez', level: risk.liquidity.cashRatio > 5 ? 'Baixo' : risk.liquidity.cashRatio > 1 ? 'Médio' : 'Alto', detail: `Caixa: ${risk.liquidity.cashRatio.toFixed(2)}% · WAM: ${risk.liquidity.wam ? risk.liquidity.wam.toFixed(1) + 'a' : 'N/A'}` },
        { category: 'Mercado (Duration)', level: risk.duration.avgDuration < 3 ? 'Baixo' : risk.duration.avgDuration < 7 ? 'Médio' : 'Alto', detail: `Duration: ${risk.duration.avgDuration.toFixed(2)}a · Impacto +100bps: ${formatBRL(risk.duration.impact100bps)}` },
        { category: 'VaR 95%', level: (risk.var.var95 / risk.pl * 100) < 3 ? 'Baixo' : (risk.var.var95 / risk.pl * 100) < 8 ? 'Médio' : 'Alto', detail: `${formatBRL(risk.var.var95)} (${(risk.var.var95 / risk.pl * 100).toFixed(2)}% PL) · horizonte 21 dias` },
      ].map((row, i) => {
...
function exportRiskReport(fund, risk) {
  if (!risk) { showToast('Sem dados de risco', true); return; }
  const d = new Date().toLocaleDateString('pt-BR');
  let md = `# Relatório de Risco: ${fund.name}\n`;
  md += `**Data:** ${d} · **PL:** ${formatBRL(risk.pl)} · **Carteira:** ${risk.fund.dtposicao}\n\n---\n\n`;

  md += `## Matriz de Risco\n\n`;
  md += `| Categoria | Nível | Detalhe |\n|---|---|---|\n`;
  ...
  md += `| Crédito (PDD) | ${risk.credit.pddCoveragePct < 5 ? 'Baixo' : risk.credit.pddCoveragePct < 15 ? 'Médio' : 'Alto'} | PDD: ${risk.credit.pddCoveragePct.toFixed(2)}% · ${risk.credit.creditCount} ativos |\n`;
  md += `| Liquidez | ${risk.liquidity.cashRatio > 5 ? 'Baixo' : risk.liquidity.cashRatio > 1 ? 'Médio' : 'Alto'} | Caixa: ${risk.liquidity.cashRatio.toFixed(2)}% |\n`;
  md += `| Mercado | ${risk.duration.avgDuration < 3 ? 'Baixo' : risk.duration.avgDuration < 7 ? 'Médio' : 'Alto'} | Duration: ${risk.duration.avgDuration.toFixed(2)}a |\n`;
  md += `| VaR 95% | ${(risk.var.var95 / risk.pl * 100) < 3 ? 'Baixo' : (risk.var.var95 / risk.pl * 100) < 8 ? 'Médio' : 'Alto'} | ${formatBRL(risk.var.var95)} (${(risk.var.var95 / risk.pl * 100).toFixed(2)}% PL) |\n\n`;
