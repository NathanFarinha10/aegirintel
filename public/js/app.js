...
async function dbLoadAll() {
  try {
    const resp = await fetch(SB_URL + '/rest/v1/shared_data?select=key,value', { headers: SB_HEADERS });
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    const rows = await resp.json();
    for (const row of rows) {
      if (SHARED_KEYS.includes(row.key) && row.value != null) DB[row.key] = row.value;
    }
    saveLocalDB();
    _dbReady = true;
    render();
