...
// === Supabase: write a shared key ===
function dbPut(key, value) {
  return fetch(SB_URL + '/rest/v1/shared_data', {
    method: 'POST', headers: { ...SB_HEADERS, 'Prefer': 'resolution=merge-duplicates' },
    body: JSON.stringify({ key, value, updated_at: new Date().toISOString() }),
  }).catch(e => console.warn('[DB] put error:', key, e.message));
}

// === Supabase: read all shared data ===
async function dbLoadAll() {
  try {
    const url = SB_URL + '/rest/v1/shared_data?select=key,value';
    const resp = await fetch(url, { headers: SB_HEADERS });
    if (!resp.ok) {
      const errText = await resp.text();
      console.warn('[DB] load HTTP ' + resp.status + ':', errText);
      showToast('Erro ao carregar dados: HTTP ' + resp.status, true);
      return false;
    }
    const rows = await resp.json();
    console.log('[DB] loaded', rows.length, 'rows from Supabase');
    if (rows.length === 0) {
      console.log('[DB] Tabela vazia — nenhum dado compartilhado ainda');
      _dbReady = true;
      return true;
    }
    for (const row of rows) {
      if (SHARED_KEYS.includes(row.key) && row.value != null) {
        DB[row.key] = row.value;
        console.log('[DB] loaded key:', row.key, typeof row.value === 'object' ? (Array.isArray(row.value) ? row.value.length + ' items' : Object.keys(row.value).length + ' keys') : typeof row.value);
      }
    }
    saveLocalDB();
    _takeSnapshot(); // snapshot after load so we don't re-push what we just downloaded
    _dbReady = true;
    render();
    return true;
  } catch (e) {
...
// Save: localStorage always + auto-detect which shared keys changed → push only those
let _dbSnapshot = {};
function _takeSnapshot() {
  for (const k of SHARED_KEYS) {
    try { _dbSnapshot[k] = JSON.stringify(DB[k]); } catch(e) { _dbSnapshot[k] = ''; }
  }
}
...
function saveDB(db) {
  saveLocalDB();
  // Detect which shared keys changed since last snapshot
  for (const k of SHARED_KEYS) {
    if (db[k] === undefined) continue;
    try {
      const current = JSON.stringify(db[k]);
      if (current !== _dbSnapshot[k]) {
        _dbSnapshot[k] = current;
        dbPut(k, db[k]);
      }
    } catch(e) {}
  }
...
// Force push ALL shared data to Supabase (manual action)
function forcePushAll() {
  for (const k of SHARED_KEYS) {
    if (DB[k] !== undefined) dbPut(k, DB[k]);
  }
  _takeSnapshot();
  showToast('Todos os dados enviados ao servidor');
}
  ...
  bg.appendChild(h('div', { class: 'modal' }, [
    ...
    h('div', { class: 'modal-body' }, [
      h('div', { style: { marginBottom: '20px' } }, [
        ...
        h('button', { class: 'btn-primary', onClick: () => { dbLoadAll().then(() => { showToast('Dados atualizados'); setModal(null); }); }}, '↓ Baixar do Servidor'),
        h('button', { class: 'btn-primary', style: { marginLeft: '8px', background: 'var(--amber)' }, onClick: () => { forcePushAll(); setModal(null); }}, '↑ Enviar ao Servidor'),
      ]),
