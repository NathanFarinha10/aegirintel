...
function renderHeader() {
  return h('header', { class: 'header' }, [
    ...
    h('div', { style: { display:'flex', gap:'10px', alignItems:'center' } }, [
      h('button', { class: 'header-btn', onClick: () => setModal('settings') }, '⚙ Settings'),
    ]),
  ]);
}
...
function renderSettingsModal() {
  ...
  const geminiKey = DB.settings?.gemini_api_key || '';
  bg.appendChild(h('div', { class: 'modal' }, [
    ...
    h('div', { class: 'modal-body' }, [
      h('div', { style: { marginBottom: '16px' } }, [
        h('label', { class: 'form-field-label' }, 'Gemini API Key'),
        h('input', { class: 'form-field-input', type: 'password', id: 'set-gemini', value: geminiKey, placeholder: 'Cole sua chave Gemini aqui' }),
        h('div', { style: { fontSize:'11px', color:'var(--text-faint)', marginTop:'4px' } }, 'Obtenha em aistudio.google.com/apikey (gratuita)'),
      ]),
      h('button', { class: 'btn-primary', onClick: () => {
        DB.settings.gemini_api_key = document.getElementById('set-gemini')?.value?.trim() || '';
        saveDB(DB);
        showToast('Settings salvas');
        setModal(null);
      }}, 'Salvar'),
    ]),
  ]));
...
function _renderInternal() {
  ...
  switch(v) {
    ...
    case 'am_new':      content = renderNewFund(); break;
