...
async function authSignUp(email, password, name) {
  ...
  // Create profile with authenticated headers
...
async function ensureProfile(userId, email, name) {
  // Try insert first
  const body = JSON.stringify({ id: userId, email, name: name || email.split('@')[0], role: 'viewer', status: 'pending' });
  let resp = await fetch(SB_URL + '/rest/v1/profiles', {
    method: 'POST',
    headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + _session.access_token, 'Content-Type': 'application/json', 'Prefer': 'resolution=merge-duplicates' },
    body,
  });
  if (!resp.ok) {
    // Try upsert with different prefer header
    resp = await fetch(SB_URL + '/rest/v1/profiles', {
      method: 'POST',
      headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + _session.access_token, 'Content-Type': 'application/json', 'Prefer': 'return=minimal' },
      body,
    });
  }
  return resp.ok;
...
async function authChangePassword(newPassword) {
  if (!_session?.access_token) throw new Error('Não autenticado');
  const resp = await fetch(SB_URL + '/auth/v1/user', {
    method: 'PUT',
    headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + _session.access_token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ password: newPassword }),
  });
  if (!resp.ok) { const e = await resp.json(); throw new Error(e.error_description || e.msg || 'Erro ao alterar senha'); }
  return true;
}
...
async function adminResetUserPassword(userId, newPassword) {
  try {
    ...
    // Note: requires service_role or admin access
    const resp = await fetch(SB_URL + '/auth/v1/admin/users/' + userId, {
      method: 'PUT',
      headers: { 'apikey': SB_KEY, 'Authorization': 'Bearer ' + _session.access_token, 'Content-Type': 'application/json' },
      body: JSON.stringify({ password: newPassword }),
    });
...
async function dbLoadAll() {
  try {
    ...
    // Reset all shared arrays to prevent duplication on re-sync
    DB.funds=[]; DB.carteirasXML={}; DB.comites={}; DB.research=[]; DB.resgateForecasts={};
    DB.assetAnalyses=[]; DB.equityAssets=[]; DB.tasks=[]; DB.liquidityHaircuts={};
    DB._dailyPipeline=DB._dailyPipeline||{}; DB._fundDates=DB._fundDates||{};
    DB._creditNotes=DB._creditNotes||{}; DB._reNotes=DB._reNotes||{};
    DB.productAnalyses=[]; DB.creditRatings=[]; DB.tradingSims=[];
    ...
    for (const row of rows) {
      ...
      else if (k.startsWith('carteira:')) {
        const parts = k.split(':');
        let cnpj = parts[1];
        // Strip class suffix from CNPJ key (e.g., '41778415000178_MZ' -> '41778415000178')
        const classMatch = cnpj.match(/^(\d+)_([A-Z]+)$/);
        if (classMatch) { cnpj = classMatch[1]; if(v?.fund) v.fund._classTag = classMatch[2]; }
        if(!carteirasXML[cnpj])carteirasXML[cnpj]=[]; carteirasXML[cnpj].push(v);
      }
      ...
      else if (k.startsWith('prodanalysis:')) { if(!DB.productAnalyses)DB.productAnalyses=[]; DB.productAnalyses.push(v); }
      else if (k.startsWith('crating:')) { if(!DB.creditRatings)DB.creditRatings=[]; DB.creditRatings.push(v); }
      else if (k.startsWith('tradesim:')) { if(!DB.tradingSims)DB.tradingSims=[]; DB.tradingSims.push(v); }
...
function saveCarteiraToServer(cnpj, carteira) { const d = carteira.fund?.dtposicao || 'unknown'; const cls = carteira.fund?.classe ? '_' + carteira.fund.classe.replace(/[^a-zA-Z]/g,'').substring(0,3).toUpperCase() : ''; sbUpsert('carteira:' + cnpj + cls + ':' + d, carteira); }
...
function setView(v) { state.view = v; state.detail = null; state._backoffice_fund = null; state._active_task = null; state._fund_edit = null; state._editing_task = null; state._bo_tab = null; state._credit_detail = null; state._re_detail = null; state._invested_fund_detail = null; render(); }
...
const SIDE_NAV = {
  ...
  gestao: [
    ...
    { group: 'Monitoria de Ativos' },
    { id: 'am_estoques',        label: 'Gestão de Estoques',  num: '01' },
  ],
  analise: [
    { group: 'Análise de Produtos' },
    { id: 'an_products', label: 'Novos Produtos',  num: '01' },
    { group: 'Análise de Ativos' },
    { id: 'an_credit',   label: 'Crédito',              num: '01' },
    { id: 'an_re',       label: 'Real Estate Equity',   num: '02' },
    { id: 'an_fundos',   label: 'Fundos',               num: '03' },
    { id: 'an_liquid',   label: 'Liquid Assets',        num: '04' },
  ],
  atividades: [
      ...
      h('nav', { style: { display:'flex', gap:'2px' } },
        TOP_TABS.map(t => h('button', {
          ...
          onClick: () => {
            ...
            state._credit_detail = null; state._re_detail = null; state._invested_fund_detail = null;
            render();
          }
        }, t.label))
      ),
...
function renderSettingsModal() {
  ...
  bg.appendChild(h('div', { class: 'modal', style: { maxWidth: isAdmin ? '600px' : '420px' } }, [
    ...
    h('div', { class: 'modal-body' }, [
      ...
      // Change Password
      h('div', { style: { borderTop:'1px solid var(--border)', paddingTop:'16px', marginBottom:'16px' } }, [
        h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'15px', marginBottom:'8px' } }, 'Alterar Senha'),
        h('div', { style: { display:'flex', gap:'8px', alignItems:'end' } }, [
          h('div', { style: { flex:1 } }, [
            h('label', { class:'form-field-label' }, 'Nova senha (mín. 6 caracteres)'),
            h('input', { class:'form-field-input', type:'password', id:'set-newpass', placeholder:'Nova senha' }),
          ]),
          h('div', { style: { flex:1 } }, [
            h('label', { class:'form-field-label' }, 'Confirmar'),
            h('input', { class:'form-field-input', type:'password', id:'set-confirmpass', placeholder:'Confirmar senha' }),
          ]),
          h('button', { class:'btn-primary', style: { height:'34px', whiteSpace:'nowrap' }, onClick: async () => {
            const p1 = document.getElementById('set-newpass')?.value;
            const p2 = document.getElementById('set-confirmpass')?.value;
            if (!p1 || p1.length < 6) { showToast('Senha deve ter pelo menos 6 caracteres', true); return; }
            if (p1 !== p2) { showToast('Senhas não coincidem', true); return; }
            try { await authChangePassword(p1); showToast('Senha alterada com sucesso'); } catch(e) { showToast(e.message, true); }
          }}, 'Alterar'),
        ]),
      ]),
      // Sync
...
function _renderInternal() {
  ...
  // Normal view routing
    ...
    switch(v) {
      ...
      case 'am_estoques':    content = renderEstoquesEmbed(); break;
      case 'an_products': content = renderAnaliseProducts(); break;
      case 'an_credit':   content = renderCreditRatingEngine(); break;
      case 'an_re':       content = renderAnaliseREWIP(); break;
      case 'an_fundos':   content = renderAnaliseFundosWIP(); break;
      case 'an_liquid':   content = renderAnaliseLiquidWIP(); break;
...
function renderComite() {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    h('div', { class: 'sec-tab-row', style: { marginBottom: '16px' } },
      [['atas', 'Atas de Comitê'], ['pretrading', 'Pré-Trading'], ['simulador', 'Simulador']].map(([k, l]) =>
    ...
    subTab === 'atas' ? renderAtasList(funds, activeFund) : subTab === 'simulador' ? renderTradingSimulator() : renderPreTradingList(funds, activeFund),
  ]);
}
...
function renderComiteForm(fundId, comite) {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    // ESG Compliance
    h('div', { class: 'card', style: { padding: '16px', marginTop: '16px' } }, [
      h('div', { style: { fontFamily: 'Fraunces, serif', fontSize: '14px', marginBottom: '12px' } }, 'Checklist de Compliance ESG'),
      (() => {
        if (!comite.esgChecklist) comite.esgChecklist = [
          { pillar: 'E', label: 'Ambiental', factor: 'Regularidade Ambiental e Eficiência', desc: 'Risco de embargos por questões de licenciamento ou passivos ambientais.', mitigation: 'Verificação de licenças (LP, LI, LO) e preferência por ativos com certificações de sustentabilidade ou gestão eficiente de resíduos e água.', checked: false, analyst: '' },
          { pillar: 'S', label: 'Social', factor: 'Saúde, Segurança e Direitos Humanos', desc: 'Risco de práticas laborais inadequadas em canteiros de obra ou na cadeia de suprimentos.', mitigation: 'Auditoria de conformidade com normas de segurança do trabalho (NRs) e verificação de listas restritivas para garantir a ausência de trabalho análogo ao escravo.', checked: false, analyst: '' },
          { pillar: 'G', label: 'Governança', factor: 'Transparência e Alinhamento', desc: 'Avaliação da integridade do gestor/emissor/parceiro e transparência na comunicação de dados.', mitigation: 'Análise do histórico do gestor/emissor e conferência dos mecanismos de segregação de funções e gestão de conflitos de interesse.', checked: false, analyst: '' },
        ];
        return h('div', {}, comite.esgChecklist.map((item, i) => {
          const pillarColors = { E: '#4a7a2c', S: '#5c7a9b', G: '#b8863c' };
          return h('div', { style: { padding: '12px 0', borderTop: i > 0 ? '1px solid var(--border)' : 'none' } }, [
            h('div', { style: { display: 'flex', gap: '12px', alignItems: 'flex-start' } }, [
              h('div', { style: { width: '36px', height: '36px', borderRadius: '6px', background: pillarColors[item.pillar] || 'var(--amber)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', fontSize: '14px', flexShrink: '0' } }, item.pillar),
              h('div', { style: { flex: 1 } }, [
                h('div', { style: { fontSize: '12px', fontWeight: '600', marginBottom: '2px' } }, item.factor),
                h('div', { style: { fontSize: '10px', color: 'var(--text-faint)', marginBottom: '4px' } }, item.desc),
                h('div', { style: { fontSize: '10px', color: 'var(--text-faint)', fontStyle: 'italic' } }, 'Mitigação: ' + item.mitigation),
              ]),
              h('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', flexShrink: '0' } }, [
                h('div', { style: { width: '24px', height: '24px', border: '2px solid ' + (item.checked ? 'var(--green)' : 'var(--border)'), borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: item.checked ? 'var(--green)' : 'transparent', color: '#fff', fontSize: '14px' },
                  onClick: () => { item.checked = !item.checked; if (item.checked) item.analyst = _user?.name || ''; else item.analyst = ''; render(); }
                }, item.checked ? '✓' : ''),
                item.checked && h('div', { class: 'mono', style: { fontSize: '8px', color: 'var(--green)' } }, item.analyst),
              ]),
            ]),
          ]);
        }));
    ...
    // Membros para Assinatura
    h('div', { class: 'card', style: { padding: '16px', marginTop: '16px' } }, [
      h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' } }, [
        h('div', { style: { fontFamily: 'Fraunces, serif', fontSize: '14px' } }, 'Membros do Comitê — Assinaturas e Votos'),
        h('button', { class: 'btn-secondary', style: { fontSize: '10px' }, onClick: () => {
          if (!comite.signatories) comite.signatories = [];
          comite.signatories.push({ id: 'sig_' + Date.now(), name: '', role: '', vote: '' });
          render();
        }}, '+ Adicionar Membro'),
      ]),
      (() => {
        if (!comite.signatories) comite.signatories = [];
        if (comite.signatories.length === 0) return h('div', { class: 'mono', style: { fontSize: '11px', color: 'var(--text-faint)', padding: '10px 0' } }, 'Nenhum membro adicionado. Clique em "+ Adicionar Membro".');
        const VOTE_OPTIONS = [
          { value: '', label: '— Selecione o voto —' },
          { value: 'approved', label: 'Aprovado sem ressalvas' },
          { value: 'approved_conditions', label: 'Aprovado, desde que cumpridas condições deliberadas em comitê' },
          { value: 'rejected_resubmit', label: 'Reprovado, com solicitação de reapresentação observando apontamentos' },
          { value: 'rejected', label: 'Reprovado' },
        ];
        return h('div', {}, comite.signatories.map((sig, i) =>
          h('div', { style: { padding: '12px 0', borderTop: i > 0 ? '1px solid var(--border)' : 'none', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 30px', gap: '10px', alignItems: 'start' } }, [
            h('div', {}, [
              h('label', { class: 'form-field-label' }, 'Nome'),
              h('input', { class: 'form-field-input', value: sig.name || '', placeholder: 'Nome completo',
                oninput: e => { sig.name = e.target.value; } }),
            ]),
            h('div', {}, [
              h('label', { class: 'form-field-label' }, 'Cargo'),
              h('input', { class: 'form-field-input', value: sig.role || '', placeholder: 'Ex: Gestor, Analista, Compliance',
                oninput: e => { sig.role = e.target.value; } }),
            ]),
            h('div', {}, [
              h('label', { class: 'form-field-label' }, 'Voto'),
              h('select', { class: 'form-field-select', style: { fontSize: '10px' },
                onchange: e => { sig.vote = e.target.value; } },
                VOTE_OPTIONS.map(o => h('option', { value: o.value, selected: sig.vote === o.value ? 'selected' : null }, o.label))
              ),
            ]),
            h('button', { class: 'mono', style: { fontSize: '12px', color: 'var(--red)', background: 'none', border: 'none', cursor: 'pointer', marginTop: '22px' },
              onClick: () => { comite.signatories.splice(i, 1); render(); }
            }, '✕'),
          ])
        ));
      })(),
...
function savePreTrading(fundId, pt) {
  ...
  saveDB(DB);
  sbUpsert('pretrading:' + fundId + ':' + pt.id, pt);
}

function deletePreTrading(fundId, ptId) {
  if (!DB.pretradings?.[fundId]) return;
  DB.pretradings[fundId] = DB.pretradings[fundId].filter(p => p.id !== ptId);
  saveDB(DB);
  sbDelete('pretrading:' + fundId + ':' + ptId);
}
  ...
  return h('div', {}, [
    ...
    pts.length === 0
      ...
      : h('div', {}, pts.map(pt => {
          ...
          return h('div', { class: 'card', style: { marginBottom: '10px', borderLeft: `3px solid ${dirColor}`, cursor: 'pointer' },
            onClick: () => { state._pretrading_editing = JSON.parse(JSON.stringify(pt)); state._pretrading_editing_fund = pt._fund_id; render(); }
          }, [
            h('div', { style: { display: 'grid', gridTemplateColumns: '60px 1fr 100px 80px 90px 60px', gap: '12px', alignItems: 'center' } }, [
              ...
              h('div', { style: { display: 'flex', gap: '4px' } }, [
                h('button', { class: 'mono', style: { fontSize: '9px', padding: '2px 6px', border: '1px solid var(--amber)', color: 'var(--amber)', background: 'none', cursor: 'pointer', borderRadius: '3px' },
                  onClick: e => { e.stopPropagation(); state._pretrading_editing = JSON.parse(JSON.stringify(pt)); state._pretrading_editing_fund = pt._fund_id; render(); }}, '✎'),
                h('button', { class: 'mono', style: { fontSize: '9px', padding: '2px 6px', border: '1px solid var(--red)', color: 'var(--red)', background: 'none', cursor: 'pointer', borderRadius: '3px' },
                  onClick: e => { e.stopPropagation(); if (confirm('Excluir pré-trading?')) { deletePreTrading(pt._fund_id, pt.id); render(); } }}, '✕'),
              ]),
            ]),
...
function parseCVMXml(xmlText) {
  ...
  fund.tipoLabel = tipo === 431 ? 'FII' : tipo === 348 ? 'FIDC' : tipo === 555 ? 'FIM' : tipo === 501 ? 'FIA' : tipo === 1353 ? 'FII' : `Tipo ${tipo}`;

  // Detect class from name (sênior, mezanino, subordinada)
  const nomeUpper = (fund.nome || '').toUpperCase();
  fund.classe = nomeUpper.includes(' SR') || nomeUpper.includes('SENIOR') || nomeUpper.includes('SÊNIOR') ? 'Sênior'
    : nomeUpper.includes(' MZ') || nomeUpper.includes('MEZANINO') || nomeUpper.includes('MEZZ') ? 'Mezanino'
    : nomeUpper.includes(' SUB') || nomeUpper.includes('SUBORDINAD') ? 'Subordinada'
    : null;
  ...
  // Consolidate positions with same codativo (multiple tranches → single position)
  function consolidatePositions(arr) {
    const grouped = {};
    for (const p of arr) {
      const key = p.codativo || p.isin || p.cnpjfundo || ('_' + Math.random());
      if (!key || key.startsWith('_')) { grouped[key] = p; continue; }
      if (!grouped[key]) {
        grouped[key] = { ...p, _tranches: 1 };
      } else {
        const g = grouped[key];
        const prevQty = g.qtdisponivel || 0;
        const addQty = p.qtdisponivel || 0;
        const prevVal = g.valorfindisp || 0;
        const addVal = p.valorfindisp || 0;
        g.qtdisponivel = prevQty + addQty;
        g.valorfindisp = prevVal + addVal;
        g.principal = (g.principal || 0) + (p.principal || 0);
        g.valorfinemgar = (g.valorfinemgar || 0) + (p.valorfinemgar || 0);
        // Weighted average PU
        if (prevQty + addQty > 0) g.puposicao = (prevVal + addVal) / (prevQty + addQty);
        // Keep worst PDD
        if ((p.percprovcred || 0) > (g.percprovcred || 0)) g.percprovcred = p.percprovcred;
        g._tranches = (g._tranches || 1) + 1;
      }
    }
    return Object.values(grouped);
  }

  positions.titprivado = consolidatePositions(positions.titprivado);
  positions.acoes = consolidatePositions(positions.acoes);
...
function getFundClasses(cnpj) {
  const carteiras = getCarteirasXML(cnpj);
  const classes = new Map();
  for (const c of carteiras) {
    const cls = c.fund?.classe || c.fund?._classTag || null;
    const key = cls || '_default';
    if (!classes.has(key)) classes.set(key, { label: cls || 'Única', tag: c.fund?._classTag || '', count: 0, latestPL: 0 });
    const entry = classes.get(key);
    entry.count++;
    if (c.fund?.patliq > entry.latestPL) entry.latestPL = c.fund.patliq;
  }
  return classes;
}

function getLatestCarteiraByClass(cnpj) {
  const carteiras = getCarteirasXML(cnpj);
  const byClass = {};
  for (const c of carteiras) {
    const cls = c.fund?.classe || c.fund?._classTag || 'Única';
    if (!byClass[cls] || (c.fund?.dtposicao||'') > (byClass[cls].fund?.dtposicao||'')) byClass[cls] = c;
  }
  return byClass;
}

function isInterclassPosition(position, fundCnpj) {
  // Check if a titprivado position is actually an interclass obligation
  // (e.g., subordinada holding sênior CRI = interclass, not real credit)
  const isin = position.isin || '';
  const cnpj = (fundCnpj || '').replace(/\D/g, '');
  // If the ISIN matches other classes of the same fund, it's interclass
  const allCarteiras = getCarteirasXML(cnpj);
  for (const c of allCarteiras) {
    if (c.fund?.classe !== (position._sourceClasse || '')) {
      const otherISINs = new Set();
      for (const p of (c.positions?.titprivado || [])) { if (p.isin) otherISINs.add(p.isin); }
      for (const p of (c.positions?.acoes || [])) { if (p.isin) otherISINs.add(p.isin); }
      if (otherISINs.has(isin) && isin !== 'BR0000000000') return true;
    }
  }
  return false;
}

function getCarteirasXML(cnpj, classFilter) {
  if (!DB.carteirasXML) DB.carteirasXML = {};
  const cleanCnpj = (cnpj||'').replace(/\D/g, '');
  if (!Array.isArray(DB.carteirasXML[cleanCnpj])) DB.carteirasXML[cleanCnpj] = [];
  if (classFilter) return DB.carteirasXML[cleanCnpj].filter(c => (c.fund?.classe||c.fund?._classTag||'') === classFilter || (c.fund?._classTag||'').startsWith(classFilter.substring(0,3).toUpperCase()));
...
function renderFundBackoffice(cnpj) {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    (() => {
      const classes = getFundClasses(fund.cnpj);
      if (classes.size > 1) {
        const currentClass = state._bo_class || null;
        return h('div', { style: { display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '12px', padding: '8px 12px', background: 'var(--bg-3)', borderRadius: '6px' } }, [
          h('span', { class: 'mono', style: { fontSize: '8px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em' } }, 'Classe:'),
          h('button', { class: 'sec-tab' + (!currentClass ? ' active' : ''), onClick: () => { state._bo_class = null; state._bo_date = null; render(); } }, 'Consolidado'),
          ...Array.from(classes.entries()).filter(([k]) => k !== '_default').map(([key, cls]) =>
            h('button', { class: 'sec-tab' + (currentClass === key ? ' active' : ''), onClick: () => { state._bo_class = key; state._bo_date = null; render(); } },
              cls.label + ' · ' + formatBRL(cls.latestPL))
          ),
        ]);
      }
      return null;
...
function renderCarteiraXMLDetail(parsed, fund) {
  ...
  return h('div', {}, [
    ...
    h('div', { style:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:'10px',marginBottom:'20px'} }, [
      ...
      (() => {
        const plCalc = (stats.totalValorDisp||0) + (stats.totalCaixa||0) + (stats.totalImoveis||0);
        const diff = (f.patliq||0) - plCalc;
        const diffPct = f.patliq > 0 ? (diff/f.patliq*100) : 0;
        const ok = Math.abs(diffPct) < 0.1;
        return renderPortKPI('PL Validação', ok ? '✓ OK' : '⚠ Δ ' + diffPct.toFixed(2) + '%', 'Calc: ' + formatBRL(plCalc));
      })(),
    ]),
    // Allocation
...
function renderCarteiraProvisoes(parsed) {
  ...
  function getProvName(code) {
    if (!DB._provNames) DB._provNames = {};
    return DB._provNames[code] || PROV_CODES_DEFAULT[code] || 'Provisão ' + code;
  }
  function setProvName(code, name) {
    if (!DB._provNames) DB._provNames = {};
    DB._provNames[code] = name;
    saveDB(DB);
    sbUpsert('misc:_provNames', DB._provNames);
  }
      ...
      h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } }, [
        h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, [
          ...
          h('tbody', {}, Object.entries(byCode).sort((a, b) => b[1].valor - a[1].valor).map(([code, data]) => {
            ...
            return h('tr', { style: { borderTop: '1px solid var(--border)' } }, [
              h('td', { style: { padding: '6px 10px', cursor: 'pointer' }, onClick: () => { const n = prompt('Nome para provisão código ' + code + ':', getProvName(code)); if (n && n.trim()) { setProvName(code, n.trim()); render(); } } }, [
                h('span', {}, getProvName(code)),
                h('span', { style: { fontSize: '9px', color: 'var(--amber)', marginLeft: '4px' } }, '✎'),
              ]),
      ...
      h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, [
        ...
        h('tbody', {}, provisoes.sort((a, b) => (b.valor || 0) - (a.valor || 0)).map(p => {
          ...
          return h('tr', { style: { borderTop: '1px solid var(--border)', background: isNew ? 'rgba(74,122,44,0.05)' : 'transparent' } }, [
            ...
            h('td', { style: { padding: '6px 10px', cursor: 'pointer' }, title: 'Clique para renomear', onClick: () => { const n = prompt('Nome para provisão código ' + p.codprov + ':', getProvName(p.codprov)); if (n && n.trim()) { setProvName(p.codprov, n.trim()); render(); } } }, [
              h('span', {}, getProvName(p.codprov)),
              h('span', { style: { fontSize: '9px', color: 'var(--amber)', marginLeft: '4px' } }, '✎'),
            ]),
...
function renderCreditPortfolio() {
  ...
  // Collect credit assets from latest carteira of each fund CLASS
  for (const fund of funds) {
    const carteiras = getCarteirasXML(fund.cnpj);
    if (carteiras.length === 0) continue;

    // Group carteiras by class and get latest of each
    const byClass = {};
    for (const c of carteiras) {
      const cls = c.fund?.classe || c.fund?._classTag || '_default';
      if (!byClass[cls] || (c.fund?.dtposicao || '') > (byClass[cls].fund?.dtposicao || '')) byClass[cls] = c;
    }

    // Multi-class fund: titprivado are interclass obligations, not real credit
    const isMultiClass = Object.keys(byClass).length > 1 || Object.keys(byClass).some(k => k !== '_default');
    if (isMultiClass) continue;

    // Single-class fund: collect titprivado as real credit assets
    for (const [cls, latest] of Object.entries(byClass)) {
      if (!latest.positions?.titprivado?.length) continue;
...
function renderCarteiraFluxo(parsed, fund) {
  ...
  if (!prev) return h('div', { style: { padding:'30px', textAlign:'center', color:'var(--text-faint)' } }, 'Necessário ao menos 2 carteiras para calcular o fluxo.');

  const plPrev = prev.fund.patliq || 0;
  const plCurr = parsed.fund.patliq || 0;
  const deltaPL = plCurr - plPrev;

  // === PERFORMANCE vs MOVIMENTAÇÕES ===
  const cotaPrev = prev.fund.valorcota || 0;
  const cotaCurr = parsed.fund.valorcota || 0;
  const qtdPrev = prev.fund.quantidade || 0;
  const qtdCurr = parsed.fund.quantidade || 0;

  // Performance = variação da cota × quantidade anterior
  const deltaPerformance = cotaPrev > 0 ? (cotaCurr - cotaPrev) * qtdPrev : 0;
  const perfPct = cotaPrev > 0 ? ((cotaCurr - cotaPrev) / cotaPrev) * 100 : 0;

  // Movimentações = variação de quantidade × cota atual
  const deltaMovimentacoes = cotaCurr > 0 ? (qtdCurr - qtdPrev) * cotaCurr : 0;
  const deltaQtd = qtdCurr - qtdPrev;
  const isAporte = deltaQtd > 0;
  const isResgate = deltaQtd < 0;

  // Cotas a emitir/resgatar (pendentes)
  const cotasEmitir = parsed.fund.vlcotasemitir || 0;
  const cotasResgatar = parsed.fund.vlcotasresgatar || 0;

  // Residual (provisões, arredondamentos, etc.)
  const residual = deltaPL - deltaPerformance - deltaMovimentacoes;

  // === DETALHAMENTO POR ATIVOS ===
  ...
  for (const p of prevAll) { const key = p.codativo||p.isin||p.cnpjfundo||''; if (!currMap[key]) exitedAssets -= (p.valorfindisp||((p.qtdisponivel||0)*(p.puposicao||0))); }

  topGains.sort((a,b) => b.delta - a.delta);
  topLosses.sort((a,b) => a.delta - b.delta);

  // === PL DE VALIDAÇÃO ===
  const plCalc = (parsed.stats?.totalValorDisp || 0) + (parsed.stats?.totalCaixa || 0) + (parsed.stats?.totalImoveis || 0);
  const plDiff = plCurr - plCalc;
  const plDiffPct = plCurr > 0 ? (plDiff / plCurr * 100) : 0;

  return h('div', {}, [
    h('div', { class:'macro-section-subhead' }, 'Decomposição do PL: ' + prev.fund.dtposicao + ' → ' + parsed.fund.dtposicao),

    // Main decomposition
    h('div', { class:'card', style: { padding:'20px', marginBottom:'20px' } }, [
      // PL Anterior
      h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'10px 0' } }, [
        h('div', { style: { flex:1, fontSize:'13px', fontWeight:'600' } }, 'PL Anterior (' + prev.fund.dtposicao + ')'),
        h('div', { class:'mono', style: { fontSize:'16px', fontWeight:'600', color:'var(--text-faint)', minWidth:'160px', textAlign:'right' } }, formatBRL(plPrev)),
      ]),

      // Performance
      h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'10px 0', borderTop:'1px solid var(--border)' } }, [
        h('div', { style: { flex:1 } }, [
          h('div', { style: { fontSize:'12px' } }, '📈 Performance (variação da cota)'),
          h('div', { class:'mono', style: { fontSize:'9px', color:'var(--text-faint)' } }, 'Cota: R$ ' + cotaPrev.toFixed(6) + ' → R$ ' + cotaCurr.toFixed(6) + ' (' + (perfPct >= 0 ? '+' : '') + perfPct.toFixed(4) + '%)'),
        ]),
        h('div', { class:'mono', style: { fontSize:'14px', fontWeight:'600', color: deltaPerformance >= 0 ? 'var(--green)' : 'var(--red)', minWidth:'160px', textAlign:'right' } }, (deltaPerformance >= 0 ? '+' : '') + formatBRL(deltaPerformance)),
      ]),

      // Movimentações
      deltaMovimentacoes !== 0 && h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'10px 0', borderTop:'1px solid var(--border)' } }, [
        h('div', { style: { flex:1 } }, [
          h('div', { style: { fontSize:'12px' } }, (isAporte ? '⬆️ Aportes (novas cotas)' : '⬇️ Resgates (saída de cotas)')),
          h('div', { class:'mono', style: { fontSize:'9px', color:'var(--text-faint)' } }, 'Qtd cotas: ' + qtdPrev.toLocaleString('pt-BR',{maximumFractionDigits:2}) + ' → ' + qtdCurr.toLocaleString('pt-BR',{maximumFractionDigits:2}) + ' (Δ ' + (deltaQtd >= 0 ? '+' : '') + deltaQtd.toLocaleString('pt-BR',{maximumFractionDigits:2}) + ')'),
        ]),
        h('div', { class:'mono', style: { fontSize:'14px', fontWeight:'600', color: deltaMovimentacoes >= 0 ? 'var(--green)' : 'var(--red)', minWidth:'160px', textAlign:'right' } }, (deltaMovimentacoes >= 0 ? '+' : '') + formatBRL(deltaMovimentacoes)),
      ]),

      // Pendentes
      (cotasEmitir > 0 || cotasResgatar > 0) && h('div', { style: { display:'flex', gap:'16px', padding:'8px 0', borderTop:'1px solid var(--border)' } }, [
        cotasEmitir > 0 && h('div', { class:'mono', style: { fontSize:'10px', color:'var(--blue)' } }, '⏳ Cotas a emitir: ' + formatBRL(cotasEmitir)),
        cotasResgatar > 0 && h('div', { class:'mono', style: { fontSize:'10px', color:'var(--amber)' } }, '⏳ Cotas a resgatar: ' + formatBRL(cotasResgatar)),
      ]),

      // Residual
      Math.abs(residual) > 1 && h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'10px 0', borderTop:'1px solid var(--border)' } }, [
        h('div', { style: { flex:1, fontSize:'12px' } }, '📋 Provisões / Ajustes / Outros'),
        h('div', { class:'mono', style: { fontSize:'14px', fontWeight:'600', color:'var(--text-faint)', minWidth:'160px', textAlign:'right' } }, (residual >= 0 ? '+' : '') + formatBRL(residual)),
      ]),

      // PL Atual
      h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'12px 0', borderTop:'2px solid var(--border)', marginTop:'4px' } }, [
        h('div', { style: { flex:1, fontSize:'14px', fontWeight:'700' } }, 'PL Atual (' + parsed.fund.dtposicao + ')'),
        h('div', { class:'mono', style: { fontSize:'18px', fontWeight:'700', color:'var(--amber)', minWidth:'160px', textAlign:'right' } }, formatBRL(plCurr)),
      ]),

      // Delta summary
      h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)', textAlign:'right', marginTop:'4px' } }, 'Δ PL: ' + (deltaPL >= 0 ? '+' : '') + formatBRL(deltaPL) + ' (' + (plPrev > 0 ? ((deltaPL/plPrev)*100).toFixed(2) : 0) + '%)'),
    ]),

    // PL de Validação
    h('div', { class:'card', style: { padding:'16px', marginBottom:'20px', borderLeft:'3px solid ' + (Math.abs(plDiffPct) < 0.1 ? 'var(--green)' : Math.abs(plDiffPct) < 1 ? 'var(--amber)' : 'var(--red)') } }, [
      h('div', { style: { display:'flex', justifyContent:'space-between', alignItems:'center' } }, [
        h('div', {}, [
          h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'14px' } }, '🔍 PL de Validação'),
          h('div', { class:'mono', style: { fontSize:'9px', color:'var(--text-faint)', marginTop:'2px' } }, 'Soma de ativos + caixa calculada pelo sistema vs PL do header XML'),
        ]),
        h('div', { style: { textAlign:'right' } }, [
          h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, 'PL Header: ' + formatBRL(plCurr)),
          h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, 'PL Calculado: ' + formatBRL(plCalc)),
          h('div', { class:'mono', style: { fontSize:'12px', fontWeight:'700', color: Math.abs(plDiffPct) < 0.1 ? 'var(--green)' : Math.abs(plDiffPct) < 1 ? 'var(--amber)' : 'var(--red)', marginTop:'4px' } }, 'Diferença: ' + formatBRL(plDiff) + ' (' + plDiffPct.toFixed(4) + '%)'),
        ]),
      ]),
      h('div', { class:'mono', style: { fontSize:'9px', color:'var(--text-faint)', marginTop:'8px', display:'flex', gap:'16px', flexWrap:'wrap' } }, [
        h('span', {}, 'Ativos: ' + formatBRL(parsed.stats?.totalValorDisp || 0)),
        h('span', {}, 'Caixa: ' + formatBRL(parsed.stats?.totalCaixa || 0)),
        h('span', {}, 'Imóveis: ' + formatBRL(parsed.stats?.totalImoveis || 0)),
        h('span', {}, 'Provisões: ' + formatBRL(parsed.stats?.totalProvisoes || 0)),
        h('span', {}, 'A Receber: ' + formatBRL(parsed.fund.valorreceber || 0)),
        h('span', {}, 'A Pagar: ' + formatBRL(parsed.fund.valorpagar || 0)),
      ]),
    ]),
...
function renderOperationalHeatmap(funds) {
  ...
  return h('div', { style: { marginBottom: '24px' } }, [
    ...
    h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } }, [
      h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
        h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
          ['Fundo', 'Status', 'Tarefas', 'Atrasadas', 'Última Cart.', 'Próx. Datas'].map((c, i) =>
...
const BRAND_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pdf-header { background: #fff; color: #1a1815; padding: 24px 32px; display: flex; justify-content: space-between; align-items: center; border-bottom: 3px solid #b8863c; }
  .pdf-header .left { display: flex; align-items: center; gap: 16px; }
  .pdf-header img { height: 40px; border-radius: 4px; }
  .pdf-header h1 { font-size: 20px; font-weight: 700; letter-spacing: -0.02em; color: #1a1815; }
...
function openPrintReport(title, subtitle, bodyHTML) {
  const logo = getLogo();
  const logoImg = logo ? '<img src="' + logo + '" style="border-radius:4px">' : '';
  const now = new Date();
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + title + '</title><style>' + BRAND_CSS + '</style></head><body>' +
    '<div class="pdf-header">' +
      ...
      '</div>' +
      '<div class="brand"><div class="date">' + dateStr + '<br>' + timeStr + '</div></div>' +
    '</div>' +
    '<div class="pdf-body">' + bodyHTML + '</div>' +
    '<div class="pdf-footer"><div class="left"><span class="conf">Confidencial</span> — Documento confidencial. Não constitui recomendação de investimento.</div><div class="right">Gerado em ' + dateStr + ' às ' + timeStr + '</div></div>' +
...
async function exportAtaPDF(comite, fund) {
  ...
  // ESG Compliance
  if (comite.esgChecklist && comite.esgChecklist.length > 0) {
    b += pSec('Compliance ESG');
    const pillarColors = { E: '#4a7a2c', S: '#5c7a9b', G: '#b8863c' };
    b += '<table><thead><tr><th>Pilar</th><th>Fator de Análise</th><th>Mitigação / Evidência</th><th class="r">Analisado</th></tr></thead><tbody>';
    for (const item of comite.esgChecklist) {
      const status = item.checked ? '<span style="color:#4a7a2c;font-weight:700;font-size:16px">✓</span>' : '<span style="color:#b73c3c">Pendente</span>';
      b += '<tr><td><strong style="color:' + (pillarColors[item.pillar]||'#333') + '">' + item.label + ' (' + item.pillar + ')</strong></td><td>' + item.factor + '<br><em style="font-size:9px;color:#999">' + item.desc + '</em></td><td style="font-size:9.5px">' + item.mitigation + '</td><td class="r">' + status + '</td></tr>';
    }
    b += '</tbody></table>';
  }
  // Signatures
  if (comite.signatories && comite.signatories.length > 0) {
    b += pSec('Assinaturas e Votos');
    const VOTE_LABELS = { approved: 'Aprovado sem ressalvas', approved_conditions: 'Aprovado com condições', rejected_resubmit: 'Reprovado — reapresentar', rejected: 'Reprovado' };
    const VOTE_COLORS = { approved: '#4a7a2c', approved_conditions: '#b8863c', rejected_resubmit: '#b73c3c', rejected: '#b73c3c' };
    b += '<table><thead><tr><th>Nome</th><th>Cargo</th><th>Voto</th><th style="width:120px">Assinatura</th></tr></thead><tbody>';
    for (const sig of comite.signatories) {
      const vc = VOTE_COLORS[sig.vote] || '#999';
      const vl = VOTE_LABELS[sig.vote] || '—';
      b += '<tr><td><strong>' + (sig.name||'—') + '</strong></td>';
      b += '<td>' + (sig.role||'—') + '</td>';
      b += '<td style="color:' + vc + ';font-weight:600;font-size:9px">' + vl + '</td>';
      b += '<td style="border-bottom:1px solid #999;height:40px"></td></tr>';
    }
    b += '</tbody></table>';
  }
...
function renderHubAdmin() {
  ...
  const active = _allUsers.filter(u => u.status !== 'pending' && u.status !== 'inactive');
  const inactive = _allUsers.filter(u => u.status === 'inactive');

  const PERM_MATRIX = [
    { action: 'Visualizar dados', viewer: true, compliance: true, analista: true, gestor: true, admin: true },
    { action: 'Criar/editar tarefas', viewer: false, compliance: true, analista: true, gestor: true, admin: true },
    { action: 'Upload carteiras', viewer: false, compliance: false, analista: true, gestor: true, admin: true },
    { action: 'Análises e ratings', viewer: false, compliance: false, analista: true, gestor: true, admin: true },
    { action: 'Editar fundos', viewer: false, compliance: false, analista: true, gestor: true, admin: true },
    { action: 'Simulações de trading', viewer: false, compliance: false, analista: true, gestor: true, admin: true },
    { action: 'Aprovar trades / comitê', viewer: false, compliance: false, analista: false, gestor: true, admin: true },
    { action: 'Criar comitê', viewer: false, compliance: false, analista: false, gestor: true, admin: true },
    { action: 'Gestão de usuários', viewer: false, compliance: false, analista: false, gestor: false, admin: true },
    { action: 'Configurações do sistema', viewer: false, compliance: false, analista: false, gestor: false, admin: true },
  ];

  function userCard(u, showActions) {
    const statusColors = { active: 'var(--green)', pending: 'var(--amber)', inactive: 'var(--text-faint)' };
    const statusLabels = { active: 'Ativo', pending: 'Pendente', inactive: 'Desativado' };
    const st = u.status || 'active';
    return h('div', { class: 'card', style: { padding: '14px 18px', marginBottom: '8px' } }, [
      h('div', { style: { display: 'flex', alignItems: 'center', gap: '12px' } }, [
        h('div', { style: { width: '38px', height: '38px', borderRadius: '50%', background: 'var(--amber)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: '700', fontSize: '15px', flexShrink: '0' } }, (u.name || '?')[0].toUpperCase()),
        h('div', { style: { flex: 1 } }, [
          h('div', { style: { fontSize: '14px', fontWeight: '500' } }, u.name || u.email),
          h('div', { class: 'mono', style: { fontSize: '10px', color: 'var(--text-faint)' } }, u.email),
          h('div', { style: { display: 'flex', gap: '6px', marginTop: '4px' } }, [
            h('span', { class: 'mono', style: { fontSize: '9px', padding: '2px 6px', border: '1px solid var(--border)', borderRadius: '3px' } }, ROLES[u.role]?.label || u.role),
            h('span', { class: 'mono', style: { fontSize: '9px', padding: '2px 6px', border: '1px solid ' + (statusColors[st]||'var(--border)'), color: statusColors[st], borderRadius: '3px' } }, statusLabels[st] || st),
          ]),
        ]),
        showActions && userCan('manage_users') && u.id !== _user?.id && h('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'flex-end' } }, [
          // Role selector
          h('select', { class: 'form-field-select', style: { fontSize: '10px', width: '140px' }, value: u.role,
            onchange: e => { updateUserRole(u.id, e.target.value).then(() => { showToast('Papel atualizado'); render(); }); }
          }, Object.entries(ROLES).map(([k, v]) => h('option', { value: k, selected: u.role === k ? 'selected' : null }, v.label))),
          // Action buttons
          h('div', { style: { display: 'flex', gap: '4px' } }, [
            st === 'pending' && h('button', { class: 'btn-primary', style: { fontSize: '9px', padding: '3px 10px' },
              onClick: () => { updateUserStatus(u.id, 'active').then(() => { showToast(u.name + ' aprovado'); render(); }); }
            }, '✓ Aprovar'),
            st === 'active' && h('button', { class: 'btn-secondary', style: { fontSize: '9px', padding: '3px 8px' },
              onClick: () => { if (confirm('Desativar ' + u.name + '?')) updateUserStatus(u.id, 'inactive').then(() => { showToast('Usuário desativado'); render(); }); }
            }, 'Desativar'),
            st === 'inactive' && h('button', { class: 'btn-secondary', style: { fontSize: '9px', padding: '3px 8px' },
              onClick: () => { updateUserStatus(u.id, 'active').then(() => { showToast('Usuário reativado'); render(); }); }
            }, 'Reativar'),
            h('button', { class: 'btn-secondary', style: { fontSize: '9px', padding: '3px 8px' },
              onClick: () => {
                const np = prompt('Nova senha para ' + u.name + ' (mín. 6 caracteres):');
                if (!np || np.length < 6) { if (np) showToast('Senha deve ter pelo menos 6 caracteres', true); return; }
                adminResetUserPassword(u.id, np).then(ok => { if (ok) showToast('Senha redefinida para ' + u.name); else showToast('Erro ao redefinir', true); });
              }
            }, 'Nova Senha'),
            h('button', { class: 'btn-secondary', style: { fontSize: '9px', padding: '3px 8px', color: 'var(--red)', borderColor: 'var(--red)' },
              onClick: () => { if (confirm('Excluir permanentemente ' + u.name + '? Esta ação não pode ser desfeita.')) adminDeleteUser(u.id).then(() => { showToast('Usuário excluído'); render(); }); }
            }, 'Excluir'),
          ].filter(Boolean)),
        ]),
      ]),
  ...
  return h('div', { class: 'content fade-up' }, [
    pageHead('Hub', '<em>Sistema</em>', 'Gestão de usuários, permissões e configurações'),

    // KPIs
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px', marginBottom: '24px' } }, [
      renderPortKPI('Total Usuários', String(_allUsers.length), ''),
      renderPortKPI('Ativos', String(active.length), ''),
      renderPortKPI('Pendentes', String(pending.length), pending.length > 0 ? 'Aguardando aprovação' : ''),
      renderPortKPI('Desativados', String(inactive.length), ''),
      renderPortKPI('Meu Papel', ROLES[_user?.role]?.label || '—', ''),
    ]),

    // Pending approvals
    pending.length > 0 && h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead', style: { color: 'var(--amber)' } }, '⏳ Aprovações Pendentes (' + pending.length + ')'),
      ...pending.map(u => userCard(u, true)),
    ]),

    // Active users
    h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead' }, 'Usuários Ativos (' + active.length + ')'),
      ...active.map(u => userCard(u, true)),
    ]),

    // Inactive users
    inactive.length > 0 && h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead' }, 'Desativados (' + inactive.length + ')'),
      ...inactive.map(u => userCard(u, true)),
    ]),

    // Permissions matrix
    h('div', { style: { marginBottom: '24px' } }, [
      h('div', { class: 'macro-section-subhead' }, 'Matriz de Permissões'),
      h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } },
        h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
          h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
            ['Ação', 'Viewer', 'Compliance', 'Analista', 'Gestor', 'Admin'].map(c =>
              h('th', { style: { padding: '7px 10px', textAlign: c === 'Ação' ? 'left' : 'center', fontFamily: 'JetBrains Mono, monospace', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-faint)' } }, c))
          )),
          h('tbody', {}, PERM_MATRIX.map(row =>
            h('tr', { style: { borderTop: '1px solid var(--border)' } }, [
              h('td', { style: { padding: '6px 10px', fontSize: '11px' } }, row.action),
              ...['viewer', 'compliance', 'analista', 'gestor', 'admin'].map(role =>
                h('td', { style: { padding: '6px 10px', textAlign: 'center', fontSize: '14px' } }, row[role] ? h('span', { style: { color: 'var(--green)' } }, '✓') : h('span', { style: { color: 'var(--border)' } }, '—'))
              ),
            ])
          )),
        ])
    ...
    // System settings
    h('div', {}, [
      h('div', { class: 'macro-section-subhead' }, 'Configurações'),
      h('div', { style: { display: 'flex', gap: '10px', flexWrap: 'wrap' } }, [
        h('button', { class: 'btn-primary', onClick: () => { dbLoadAll().then(() => showToast('Dados sincronizados')); }}, '↓ Sincronizar Dados'),
        h('button', { class: 'btn-secondary', onClick: () => setModal('settings') }, '⚙ Configurações'),
      ]),
    ]),
  ]);
}

function renderAnaliseProducts() {
  const analyses = getProductAnalyses();
  const PRODUCT_TYPES = {
    fidc: { label: 'FIDC', icon: '📊', desc: 'Fundo de Investimento em Direitos Creditórios', hasEngine: true },
    fii:  { label: 'FII',  icon: '🏢', desc: 'Fundo de Investimento Imobiliário', hasEngine: false },
    fim:  { label: 'FIM',  icon: '📈', desc: 'Fundo de Investimento Multimercado', hasEngine: false },
    fiagro: { label: 'FI-Agro', icon: '🌾', desc: 'Fundo de Investimento em Cadeias Agroindustriais', hasEngine: false },
    fip:  { label: 'FIP',  icon: '🏗️', desc: 'Fundo de Investimento em Participações', hasEngine: false },
    etf:  { label: 'ETF',  icon: '📉', desc: 'Exchange Traded Fund', hasEngine: false },
  };
  const STATUS_LABELS = { draft: 'Rascunho', analyzing: 'Em Análise', review: 'Em Revisão', approved: 'Aprovado', rejected: 'Rejeitado', archived: 'Arquivado' };
  const STATUS_COLORS = { draft: 'var(--text-faint)', analyzing: 'var(--blue)', review: 'var(--amber)', approved: 'var(--green)', rejected: 'var(--red)', archived: 'var(--text-faint)' };

  // Detail view
  if (state._product_detail) {
    const a = analyses.find(x => x.id === state._product_detail);
    if (!a) { state._product_detail = null; return renderAnaliseProducts(); }
    const pt = PRODUCT_TYPES[a.type] || {};
    return h('div', { class: 'content fade-up' }, [
      h('button', { class: 'back-btn', onClick: () => { state._product_detail = null; render(); } }, [h('span', {}, '←'), h('span', {}, 'Voltar')]),
      h('div', { style: { display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'20px' } }, [
        h('div', {}, [
          h('div', { class:'mono', style: { fontSize:'10px', color:'var(--amber)', textTransform:'uppercase' } }, (pt.icon||'') + ' ' + (pt.label||a.type)),
          h('h1', { style: { fontFamily:'Fraunces,serif', fontSize:'22px', margin:'4px 0' } }, a.name),
          h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, 'Por ' + (a.analyst||'—') + ' · ' + (a.created_at||'').split('T')[0]),
        ]),
        h('div', { style: { display:'flex', gap:'8px', alignItems:'center' } }, [
          h('select', { class:'form-field-select', style: { fontSize:'10px', width:'120px' }, value: a.status, onchange: e => { a.status = e.target.value; a.updated_at = new Date().toISOString(); saveProductAnalysis(a); render(); } },
            Object.entries(STATUS_LABELS).map(([k,v]) => h('option', { value: k, selected: a.status === k ? 'selected' : null }, v))),
          h('button', { class:'btn-secondary', style: { color:'var(--red)', fontSize:'10px' }, onClick: () => { if (confirm('Excluir análise?')) { deleteProductAnalysis(a.id); state._product_detail = null; render(); } }}, 'Excluir'),
        ]),
      ]),
      // Summary
      h('div', { class:'card', style: { padding:'16px', marginBottom:'16px' } }, [
        h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'14px', marginBottom:'8px' } }, 'Resumo da Análise'),
        h('textarea', { class:'form-field-input', id:'pa-summary', rows:'4', style: { resize:'vertical', width:'100%' } }, a.summary || ''),
        h('button', { class:'btn-primary', style: { marginTop:'8px' }, onClick: () => { a.summary = document.getElementById('pa-summary')?.value||''; a.updated_at = new Date().toISOString(); saveProductAnalysis(a); showToast('Salvo'); }}, 'Salvar'),
      ]),
      // FIDC engine link
      pt.hasEngine && h('div', { class:'card', style: { padding:'16px', marginBottom:'16px', borderLeft:'3px solid var(--amber)' } }, [
        h('div', { style: { display:'flex', justifyContent:'space-between', alignItems:'center' } }, [
          h('div', {}, [
            h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'14px' } }, 'Motor de Viabilidade — ' + pt.label),
            h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, 'Simulador completo de estruturação'),
          ]),
          h('button', { class:'btn-primary', onClick: () => { window.open('/viabilidade.html', '_blank'); }}, 'Abrir Simulador ↗'),
        ]),
      ]),
      !pt.hasEngine && h('div', { class:'card', style: { padding:'20px', textAlign:'center', color:'var(--text-faint)' } }, [
        h('div', { style: { fontSize:'24px', marginBottom:'6px' } }, '🚧'),
        h('div', { class:'mono', style: { fontSize:'11px' } }, 'Motor de viabilidade para ' + (pt.label||a.type) + ' em desenvolvimento'),
      ]),
      // Comments
      h('div', { style: { marginTop:'16px' } }, [
        h('div', { class:'macro-section-subhead' }, 'Discussão (' + (a.comments||[]).length + ')'),
        ...(a.comments||[]).map(c => h('div', { class:'card', style: { padding:'10px 14px', marginBottom:'4px' } }, [
          h('div', { style: { display:'flex', justifyContent:'space-between', marginBottom:'4px' } }, [
            h('span', { class:'mono', style: { fontSize:'10px', fontWeight:'600', color:'var(--amber)' } }, c.author),
            h('span', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, (c.date||'').split('T')[0]),
          ]),
        ...
        h('div', { style: { display:'flex', gap:'8px', marginTop:'8px' } }, [
          h('input', { class:'form-field-input', id:'pa-comment', style: { flex:1 }, placeholder:'Adicionar comentário...' }),
          h('button', { class:'btn-primary', onClick: () => {
            const text = document.getElementById('pa-comment')?.value?.trim();
            if (!text) return;
            if (!a.comments) a.comments = [];
            a.comments.push({ author: _user?.name||'Analista', text, date: new Date().toISOString() });
            saveProductAnalysis(a); render();
          }}, 'Enviar'),
        ]),
      ]),
    ]);
  ...
  // List view
  return h('div', { class: 'content fade-up' }, [
    pageHead('Análise', 'Novos <em>Produtos</em>', 'Pipeline de análise de viabilidade de novos fundos'),

    // New analysis picker
    h('div', { style: { marginBottom:'24px' } }, [
      h('div', { class:'macro-section-subhead' }, 'Nova Análise'),
      h('div', { style: { display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:'10px' } },
        Object.entries(PRODUCT_TYPES).map(([key, pt]) =>
          h('div', { class:'card', style: { padding:'14px', cursor:'pointer', borderLeft: pt.hasEngine ? '3px solid var(--amber)' : '3px solid var(--border)', textAlign:'center' },
            onClick: () => {
              const a = { id: 'pa_'+Date.now(), type: key, name: 'Novo ' + pt.label, status: 'draft', summary: '', analyst: _user?.name||'', comments: [], created_at: new Date().toISOString(), updated_at: new Date().toISOString() };
              saveProductAnalysis(a);
              state._product_detail = a.id;
              render();
            }
          }, [
            h('div', { style: { fontSize:'24px', marginBottom:'4px' } }, pt.icon),
            h('div', { style: { fontSize:'12px', fontWeight:'600' } }, pt.label),
            h('div', { class:'mono', style: { fontSize:'8px', color: pt.hasEngine ? 'var(--amber)' : 'var(--text-faint)', marginTop:'2px' } }, pt.hasEngine ? 'Motor disponível' : 'Em breve'),
          ])
        )
      ),
    ]),
    ...
    // Existing analyses
    analyses.length > 0 && h('div', {}, [
      h('div', { class:'macro-section-subhead' }, 'Análises (' + analyses.length + ')'),
      h('div', { class:'card', style: { padding:0 } },
        analyses.sort((a,b) => (b.updated_at||'').localeCompare(a.updated_at||'')).map((a, i) => {
          const pt = PRODUCT_TYPES[a.type] || {};
          return h('div', { style: { display:'flex', alignItems:'center', gap:'12px', padding:'12px 16px', borderTop: i>0?'1px solid var(--border)':'none', cursor:'pointer' },
            onClick: () => { state._product_detail = a.id; render(); }
          }, [
            h('div', { style: { fontSize:'20px', flexShrink:'0' } }, pt.icon || '📋'),
            h('div', { style: { flex:1 } }, [
              h('div', { style: { fontSize:'13px', fontWeight:'500' } }, a.name),
              h('div', { class:'mono', style: { fontSize:'10px', color:'var(--text-faint)' } }, (pt.label||a.type) + ' · ' + (a.analyst||'—') + ' · ' + (a.updated_at||'').split('T')[0]),
            ]),
            h('span', { class:'mono', style: { fontSize:'9px', padding:'3px 8px', border:'1px solid '+(STATUS_COLORS[a.status]||'var(--border)'), color: STATUS_COLORS[a.status]||'var(--text-faint)', borderRadius:'4px' } }, STATUS_LABELS[a.status]||a.status),
          ]);
        })
      ),
    ]),
...
function renderAssetFundosWIP() {
  ...
  const byInvestedFund = {};

  for (const f of funds) {
    const cs = getCarteirasXML(f.cnpj);
    const lt = cs.length > 0 ? cs[cs.length - 1] : null;
    if (!lt || !lt.positions?.cotas) continue;

    for (const c of lt.positions.cotas) {
      const key = c.cnpjfundo || c.isin || c.codativo || '';
      if (!key) continue;
      const val = c.valorfindisp || ((c.qtdisponivel || 0) * (c.puposicao || 0));
      const entry = { ...c, fundName: f.name, fundCnpj: f.cnpj, fundId: f.id, valorCalc: val, dtposicao: lt.fund.dtposicao };
      allInvestments.push(entry);

      if (!byInvestedFund[key]) byInvestedFund[key] = { key, name: c.codativo || c.cnpjfundo || '', cnpj: c.cnpjfundo || '', totalValue: 0, totalQtd: 0, holders: [], isZeragem: isZeragemFund(key) };
      byInvestedFund[key].totalValue += val;
      byInvestedFund[key].totalQtd += (c.qtdisponivel || 0);
      byInvestedFund[key].holders.push({ fundName: f.name, value: val, qtd: c.qtdisponivel || 0, pu: c.puposicao || 0 });
    }
  }

  const investedFunds = Object.values(byInvestedFund).sort((a, b) => b.totalValue - a.totalValue);
  const totalInvested = investedFunds.reduce((a, f) => a + f.totalValue, 0);
  const zeragemFunds = investedFunds.filter(f => f.isZeragem);
  const nonZeragem = investedFunds.filter(f => !f.isZeragem);
  const totalZeragem = zeragemFunds.reduce((a, f) => a + f.totalValue, 0);
  const totalNonZeragem = nonZeragem.reduce((a, f) => a + f.totalValue, 0);
  const uniqueHolders = new Set(allInvestments.map(i => i.fundCnpj)).size;

  // Detail view
  if (state._invested_fund_detail) {
    const detail = byInvestedFund[state._invested_fund_detail];
    if (!detail) { state._invested_fund_detail = null; return renderAssetFundosWIP(); }
    return h('div', { class: 'content fade-up' }, [
      h('button', { class: 'back-btn', onClick: () => { state._invested_fund_detail = null; render(); } }, [h('span', {}, '←'), h('span', {}, 'Voltar ao Portfólio')]),
      h('div', { style: { marginBottom: '20px' } }, [
        h('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '4px' } }, [
          detail.isZeragem && h('span', { class: 'mono', style: { fontSize: '8px', padding: '2px 6px', background: 'var(--bg-3)', borderRadius: '3px', color: 'var(--text-faint)' } }, 'ZERAGEM'),
          h('div', { class: 'mono', style: { fontSize: '10px', color: 'var(--amber)', textTransform: 'uppercase' } }, 'Fundo Investido'),
        ]),
        h('h1', { style: { fontFamily: 'Fraunces, serif', fontSize: '22px', margin: '4px 0' } }, detail.name || detail.cnpj),
        detail.cnpj && h('div', { class: 'mono', style: { fontSize: '10px', color: 'var(--text-faint)' } }, 'CNPJ: ' + detail.cnpj),
      ]),

      h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', marginBottom: '24px' } }, [
        renderPortKPI('Valor Total', formatBRL(detail.totalValue), ''),
        renderPortKPI('% do Total', (detail.totalValue / (totalInvested || 1) * 100).toFixed(2) + '%', 'de investimentos em fundos'),
        renderPortKPI('Quantidade Total', detail.totalQtd.toLocaleString('pt-BR', { maximumFractionDigits: 6 }), ''),
        renderPortKPI('Fundos Detentores', String(detail.holders.length), ''),
        detail.totalQtd > 0 && renderPortKPI('PU Médio', 'R$ ' + (detail.totalValue / detail.totalQtd).toFixed(6), ''),
      ].filter(Boolean)),

      // Holders table
      h('div', { class: 'macro-section-subhead' }, 'Fundos que Detêm esta Posição'),
      h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } },
        h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
          h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
            ['Fundo Detentor', 'Quantidade', 'PU', 'Valor', '% da Posição'].map((c, i) =>
              h('th', { style: { padding: '7px 10px', textAlign: i === 0 ? 'left' : 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-faint)' } }, c)))),
          h('tbody', {}, detail.holders.sort((a, b) => b.value - a.value).map(holder =>
            h('tr', { style: { borderTop: '1px solid var(--border)' } }, [
              h('td', { style: { padding: '6px 10px', fontWeight: '500' } }, holder.fundName),
              h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, holder.qtd.toLocaleString('pt-BR', { maximumFractionDigits: 6 })),
              h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, 'R$ ' + holder.pu.toFixed(6)),
              h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right', fontWeight: '600' } }, formatBRL(holder.value)),
              h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right', color: 'var(--text-faint)' } }, (holder.value / detail.totalValue * 100).toFixed(1) + '%'),
            ])
          )),
        ])
      ),
  ...
  // List view
  return h('div', { class: 'content fade-up' }, [
    pageHead('Gestão de Ativos', 'Portfólio de <em>Fundos</em>', 'Visão consolidada de cotas de fundos investidos. ' + investedFunds.length + ' fundos em ' + uniqueHolders + ' carteira(s).'),

    investedFunds.length === 0
      ? h('div', { class: 'empty' }, [h('div', { class: 'empty-title' }, 'Nenhuma cota de fundo encontrada nas carteiras')])
      : h('div', {}, [
        // KPIs
        h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px', marginBottom: '24px' } }, [
          renderPortKPI('Total Investido', formatBRL(totalInvested), investedFunds.length + ' fundos'),
          renderPortKPI('Fundos Estratégicos', formatBRL(totalNonZeragem), nonZeragem.length + ' fundo(s)'),
          renderPortKPI('Zeragem / Liquidez', formatBRL(totalZeragem), zeragemFunds.length + ' fundo(s)'),
          renderPortKPI('Concentração Top 3', investedFunds.length >= 3 ? (investedFunds.slice(0, 3).reduce((a, f) => a + f.totalValue, 0) / (totalInvested || 1) * 100).toFixed(1) + '%' : '—', ''),
        ]),

        // Allocation chart
        (() => {
          const top = nonZeragem.slice(0, 8);
          if (top.length === 0) return null;
          const maxVal = top[0]?.totalValue || 1;
          return h('div', { style: { marginBottom: '24px' } }, [
            h('div', { class: 'macro-section-subhead' }, 'Alocação por Fundo (estratégicos)'),
            h('div', { class: 'card', style: { padding: '16px' } },
              top.map((f, i) => {
                const pct = f.totalValue / (totalInvested || 1) * 100;
                const barW = f.totalValue / maxVal * 100;
                return h('div', { style: { marginBottom: i < top.length - 1 ? '10px' : 0 } }, [
                  h('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: '3px' } }, [
                    h('span', { class: 'mono', style: { fontSize: '10px', fontWeight: '500' } }, f.name || f.cnpj),
                    h('span', { class: 'mono', style: { fontSize: '10px', color: 'var(--text-faint)' } }, formatBRL(f.totalValue) + ' · ' + pct.toFixed(1) + '%'),
                  ]),
                  h('div', { style: { height: '8px', background: 'var(--bg-3)', borderRadius: '4px', overflow: 'hidden' } }, [
                    h('div', { style: { width: barW + '%', height: '100%', background: pct > 15 ? 'var(--amber)' : 'var(--green)', borderRadius: '4px', transition: 'width 0.3s' } }),
                  ]),
                ]);
              })
            ),
        ...
        // Non-zeragem table
        nonZeragem.length > 0 && h('div', { style: { marginBottom: '24px' } }, [
          h('div', { class: 'macro-section-subhead' }, 'Fundos Estratégicos (' + nonZeragem.length + ')'),
          h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } },
            h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
              h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
                ['Fundo/CNPJ', 'Detentores', 'Qtd Total', 'Valor Total', '% Total'].map((c, i) =>
                  h('th', { style: { padding: '7px 10px', textAlign: i === 0 ? 'left' : 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-faint)' } }, c)))),
              h('tbody', {}, nonZeragem.map(f => {
                const pct = f.totalValue / (totalInvested || 1) * 100;
                return h('tr', { style: { borderTop: '1px solid var(--border)', cursor: 'pointer' }, onClick: () => { state._invested_fund_detail = f.key; render(); } }, [
                  h('td', { style: { padding: '6px 10px' } }, [
                    h('div', { style: { fontWeight: '500' } }, f.name || '(sem nome)'),
                    f.cnpj && h('div', { class: 'mono', style: { fontSize: '9px', color: 'var(--text-faint)' } }, f.cnpj),
                  ]),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, String(f.holders.length)),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, f.totalQtd.toLocaleString('pt-BR', { maximumFractionDigits: 2 })),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right', fontWeight: '600' } }, formatBRL(f.totalValue)),
                  h('td', { style: { padding: '6px 10px', textAlign: 'right' } }, [
                    h('span', { class: 'mono', style: { fontSize: '10px' } }, pct.toFixed(1) + '%'),
                    h('div', { style: { height: '3px', background: 'var(--bg-3)', borderRadius: '2px', marginTop: '2px', width: '50px', marginLeft: 'auto' } }, [
                      h('div', { style: { width: Math.min(100, pct * 2) + '%', height: '100%', background: 'var(--amber)', borderRadius: '2px' } }),
                    ]),
                  ]),
                ]);
              })),
        ...
        // Zeragem table
        zeragemFunds.length > 0 && h('div', {}, [
          h('div', { class: 'macro-section-subhead' }, 'Fundos de Zeragem / Liquidez (' + zeragemFunds.length + ')'),
          h('div', { class: 'card', style: { padding: 0, overflow: 'auto' } },
            h('table', { style: { width: '100%', borderCollapse: 'collapse', fontSize: '11px' } }, [
              h('thead', {}, h('tr', { style: { background: 'var(--bg-3)' } },
                ['Fundo/CNPJ', 'Detentores', 'Valor Total', '% Total'].map((c, i) =>
                  h('th', { style: { padding: '7px 10px', textAlign: i === 0 ? 'left' : 'right', fontFamily: 'JetBrains Mono, monospace', fontSize: '8px', textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-faint)' } }, c)))),
              h('tbody', {}, zeragemFunds.map(f =>
                h('tr', { style: { borderTop: '1px solid var(--border)', cursor: 'pointer' }, onClick: () => { state._invested_fund_detail = f.key; render(); } }, [
                  h('td', { style: { padding: '6px 10px' } }, [
                    h('div', { style: { fontWeight: '500', color: 'var(--text-faint)' } }, f.name || '(sem nome)'),
                    f.cnpj && h('div', { class: 'mono', style: { fontSize: '9px', color: 'var(--text-faint)' } }, f.cnpj),
                  ]),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, String(f.holders.length)),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right' } }, formatBRL(f.totalValue)),
                  h('td', { class: 'mono', style: { padding: '6px 10px', textAlign: 'right', color: 'var(--text-faint)' } }, (f.totalValue / (totalInvested || 1) * 100).toFixed(1) + '%'),
                ])
              )),
            ])
          ),
...
function getProductAnalyses() { if (!Array.isArray(DB.productAnalyses)) DB.productAnalyses = []; return DB.productAnalyses; }

function saveProductAnalysis(a) {
  if (!Array.isArray(DB.productAnalyses)) DB.productAnalyses = [];
  a.updated_at = new Date().toISOString();
  const idx = DB.productAnalyses.findIndex(x => x.id === a.id);
  if (idx >= 0) DB.productAnalyses[idx] = a;
  else DB.productAnalyses.push(a);
  saveDB(DB);
  sbUpsert('prodanalysis:' + a.id, a);
}

function deleteProductAnalysis(id) {
  DB.productAnalyses = getProductAnalyses().filter(x => x.id !== id);
  saveDB(DB);
  sbDelete('prodanalysis:' + id);
}

// WIP analysis views
function renderAnaliseREWIP() {
  return h('div', { class: 'content fade-up' }, [
    pageHead('Análise de Ativos', 'Real Estate <em>Equity</em>', 'Análise de novos ativos imobiliários e empreendimentos'),
    h('div', { class: 'card', style: { padding:'40px', textAlign:'center' } }, [
      h('div', { style: { fontSize:'40px', marginBottom:'12px' } }, '🚧'),
      h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'18px', marginBottom:'8px' } }, 'Em Desenvolvimento'),
      h('div', { class:'mono', style: { fontSize:'11px', color:'var(--text-faint)', maxWidth:'400px', margin:'0 auto' } },
        'Pipeline de análise de ativos imobiliários para alocação. Em breve.'),
    ]),
  ]);
}

function renderAnaliseFundosWIP() {
  return h('div', { class: 'content fade-up' }, [
    pageHead('Análise de Ativos', '<em>Fundos</em>', 'Análise de fundos para investimento'),
    h('div', { class: 'card', style: { padding:'40px', textAlign:'center' } }, [
      h('div', { style: { fontSize:'40px', marginBottom:'12px' } }, '🚧'),
      h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'18px', marginBottom:'8px' } }, 'Em Desenvolvimento'),
      h('div', { class:'mono', style: { fontSize:'11px', color:'var(--text-faint)', maxWidth:'400px', margin:'0 auto' } },
        'Pipeline de análise de fundos de investimento para alocação. Em breve.'),
    ]),
  ]);
}

function renderAnaliseLiquidWIP() {
  return h('div', { class: 'content fade-up' }, [
    pageHead('Análise de Ativos', '<em>Liquid Assets</em>', 'Análise de ativos líquidos'),
    h('div', { class: 'card', style: { padding:'40px', textAlign:'center' } }, [
      h('div', { style: { fontSize:'40px', marginBottom:'12px' } }, '🚧'),
      h('div', { style: { fontFamily:'Fraunces,serif', fontSize:'18px', marginBottom:'8px' } }, 'Em Desenvolvimento'),
      h('div', { class:'mono', style: { fontSize:'11px', color:'var(--text-faint)', maxWidth:'400px', margin:'0 auto' } },
        'Pipeline de análise de títulos públicos e ativos líquidos. Em breve.'),
    ]),
  ]);
}


// Credit Rating Engine loaded from rating.js


function renderEstoquesEmbed() {
  return h('div', { class: 'content', style: { padding: 0, display: 'flex', flexDirection: 'column', height: 'calc(100vh - 48px)', overflow: 'hidden' } }, [
    h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 20px', borderBottom: '1px solid var(--border)', flexShrink: 0 } }, [
      h('div', {}, [
        h('div', { style: { fontFamily: 'Fraunces, serif', fontSize: '14px' } }, 'Gestão de Estoques'),
        h('div', { class: 'mono', style: { fontSize: '9px', color: 'var(--text-faint)' } }, 'Estoque imobiliário integrado'),
      ]),
      h('button', { class: 'btn-secondary', style: { fontSize: '10px' }, onClick: () => window.open('/estoques.html', '_blank') }, 'Abrir em nova aba ↗'),
    ]),
    h('iframe', { src: '/estoques.html', style: { flex: 1, width: '100%', border: 'none', minHeight: 0 } }),
  ]);
}
