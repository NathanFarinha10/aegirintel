...
// Write a shared key to KV immediately
function kvPut(key, value) {
  fetch('/api/data/' + encodeURIComponent(key), {
    method: 'PUT', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(value),
  }).catch(err => console.warn('[KV] put error:', key, err.message));
}

// Save DB: localStorage + push shared keys to KV instantly
function saveDB(db) {
  saveLocalDB();
  for (const key of SHARED_KEYS) {
    if (db[key] !== undefined) kvPut(key, db[key]);
  }
}

// Boot: load all shared data from KV
async function loadFromKV() {
  try {
    ...
    for (const key of SHARED_KEYS) {
      if (shared[key] !== undefined) DB[key] = shared[key];
    }
    saveLocalDB();
    _kvReady = true;
    render();
