...
const SIDE_NAV = {
  hub: [
    { id: 'hub_geral',     label: 'Dados Gerais',    num: '01' },
    { id: 'hub_operac',    label: 'Operacional',     num: '02' },
    { id: 'hub_admin',     label: 'Administração',   num: '03' },
  ],
  gestao: [
    { id: 'am_funds',           label: 'Fundos',              num: '01' },
    { id: 'am_comite',          label: 'Comitê / Trading',    num: '02' },
    { id: 'am_product',         label: 'Product Analysis',    num: '03' },
    { id: 'am_assets',          label: 'Assets Analysis',     num: '04' },
    { id: 'am_credit_port',     label: 'Credit Portfolio',    num: '05' },
    { id: 'am_realestate_port', label: 'Real Estate Portfolio',num: '06' },
    { id: 'am_equity_port',     label: 'Equity Portfolio',    num: '07' },
    { id: 'am_new',             label: '+ Novo Fundo',        num: '08' },
  ],
  analise: [
...
function renderSidebar() {
  ...
  return h('aside', { class: 'sidebar' }, [
    h('div', { class: 'sidebar-section' }, [
      h('div', { class: 'sidebar-label' }, (TOP_TABS.find(t => t.id === state.topTab)?.label || '').toUpperCase()),
...
function renderFundBackoffice(cnpj) {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    (() => { try {
      ...
      state._bo_risk = riskCarteira ? computeRiskAnalysis(riskCarteira) : null;
    ...
    h('div', { style: { marginBottom: '16px' } }, [
      h('div', { style: { display:'flex', alignItems:'center', gap:'6px', flexWrap:'wrap' } }, [
        ...
        ...['panorama', 'carteira', 'rentabilidade', 'fluxo', 'pipeline', 'provisoes', 'imoveis', 'datas'].map(k => {
          const labels = { panorama:'Panorama', carteira:'Carteira', rentabilidade:'Rentabilidade', fluxo:'Fluxo', pipeline:'Pipeline', provisoes:'Provisões', imoveis:'Imóveis', datas:'Datas' };
        ...
        ...(() => {
          const r = state._bo_risk;
          const fp = r?.fundProfile || {};
          return [
            ['r_overview', 'Resumo'],
            ['r_concentration', 'Concentração'],
            fp.hasCredit && ['r_credit', 'Crédito'],
            fp.hasCredit && ['r_lgd', 'LGD'],
            ['r_liquidity', 'Liquidez'],
            ['r_stress', 'Stress'],
            ['r_compliance', 'CVM 175'],
          ].filter(Boolean).map(([k, l]) => h('button', { class:'sec-tab' + (subTab === k ? ' active' : ''), onClick: () => { state._bo_tab = k; render(); } }, l));
        })(),
      ]),
    ]),
    ...
    !selected && subTab !== 'panorama'
      ...
      : subTab === 'panorama' ? renderFundPanorama(fund, carteiras) :
        ...
        subTab === 'datas' ? renderFundDates(fund) :
        ...
        subTab === 'r_credit' ? (state._bo_risk ? renderRiskCredit(state._bo_risk) : null) :
        subTab === 'r_lgd' ? (state._bo_risk ? renderRiskLGD(state._bo_risk) : null) :
        ...
        subTab === 'r_compliance' ? (state._bo_risk ? renderRiskCompliance(state._bo_risk, selected) : null) :
        null,
  ]);
}
...
function renderHubGeral() {
  const funds = getFunds();
  const carteiras = {};
  let totalPL = 0, totalFunds = funds.length, fundsWithCarteira = 0;
  for (const f of funds) {
    const cs = getCarteirasXML(f.cnpj);
    carteiras[f.id] = cs;
    if (cs.length > 0) { fundsWithCarteira++; totalPL += cs[cs.length-1].fund.patliq || 0; }
  }
  const tasks = getTasks();
  const pendingTasks = tasks.filter(t => t.status !== 'done' && t.status !== 'cancelled').length;
  const users = _allUsers.length;

  return h('div', { class: 'content fade-up' }, [
    pageHead('Hub', 'Dados <em>Gerais</em>', 'Visão consolidada da gestora'),
    h('div', { style: { display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:'10px', marginBottom:'24px' } }, [
      renderPortKPI('Fundos', String(totalFunds), ''),
      renderPortKPI('PL Consolidado', formatBRL(totalPL), ''),
      renderPortKPI('Com Carteira', fundsWithCarteira + '/' + totalFunds, ''),
      renderPortKPI('Tarefas Ativas', String(pendingTasks), ''),
      renderPortKPI('Usuários', String(users), ''),
    ]),
    // Fund summary table
    ...
    h('div', { class:'card', style: { padding:0, overflow:'auto' } },
      h('table', { style: { width:'100%', borderCollapse:'collapse', fontSize:'11px' } }, [
        h('thead', {}, h('tr', { style: { background:'var(--bg-3)' } },
          ['Fundo','Classificação','PL','Cota','Carteiras','Última'].map((c,i) =>
            h('th', { style: { padding:'7px 10px', textAlign: i===0?'left':'right', fontFamily:'JetBrains Mono,monospace', fontSize:'8px', textTransform:'uppercase', letterSpacing:'0.1em', color:'var(--text-faint)' } }, c)))),
        h('tbody', {}, funds.map(f => {
          const cs = carteiras[f.id] || [];
          const lt = cs.length > 0 ? cs[cs.length-1] : null;
          return h('tr', { style: { borderTop:'1px solid var(--border)', cursor:'pointer' }, onClick: () => { state.topTab = 'gestao'; state.view = 'am_funds'; state._backoffice_fund = f.cnpj || f.id; render(); } }, [
            h('td', { style: { padding:'6px 10px', fontWeight:'500' } }, f.name),
            h('td', { class:'mono', style: { padding:'6px 10px', textAlign:'right', color:'var(--text-faint)', fontSize:'10px' } }, f.classification || '—'),
            h('td', { class:'mono', style: { padding:'6px 10px', textAlign:'right', fontWeight:'600' } }, lt ? formatBRL(lt.fund.patliq) : '—'),
            h('td', { class:'mono', style: { padding:'6px 10px', textAlign:'right' } }, lt?.fund?.valorcota ? 'R$ '+lt.fund.valorcota.toFixed(6) : '—'),
            h('td', { class:'mono', style: { padding:'6px 10px', textAlign:'right' } }, String(cs.length)),
            h('td', { class:'mono', style: { padding:'6px 10px', textAlign:'right', color:'var(--text-faint)' } }, lt?.fund?.dtposicao || '—'),
          ]);
        })),
      ])
    ),
