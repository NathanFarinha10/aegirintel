...
function initPDF(title, orientation) {
  if (!window.jspdf?.jsPDF) {
    showToast('Carregando biblioteca PDF, tente novamente em 2 segundos...', true);
...
// Thesis Report
function exportTesePDF(tese, fund) {
  const doc = initPDF('Tese de Investimento'); if (!doc) return;
  const st = THESIS_STATUS[tese.status] || {};
  const conv = CONVICTION_LEVELS[tese.conviction] || {};
...
function exportAtaPDF(comite, fund) {
  const doc = initPDF('Ata de Comitê'); if (!doc) return;
...
// Carteira Report
function exportCarteiraPDF(parsed, fund) {
  const doc = initPDF('Relatório de Carteira', 'landscape'); if (!doc) return;
  const f = parsed.fund;
  const stats = parsed.stats || {};
...
function exportFundOverviewPDF(fund) {
  const doc = initPDF('Relatório do Fundo'); if (!doc) return;
  const carteiras = getCarteirasXML(fund.cnpj);
  const latest = carteiras.length > 0 ? carteiras[carteiras.length - 1] : null;
...
// === Risk Report PDF ===
function exportRiskReportPDF(fund, risk, carteira) {
  if (!risk) { showToast('Sem dados de risco', true); return; }
  const doc = initPDF('Relatório de Risco', 'portrait'); if (!doc) return;
  const fp = risk.fundProfile || {};
...
// === Asset Analysis PDF ===
function exportAssetAnalysisPDF(a) {
  if (!a) return;
  const doc = initPDF('Análise de Ativo'); if (!doc) return;
