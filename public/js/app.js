...
// Save: localStorage always + KV with single atomic write
function saveDB(db) {
  saveLocalDB();
  if (_saving) return; // prevent overlapping saves
  _saving = true;
  const shared = {};
  for (const k of SHARED_KEYS) { if (db[k] !== undefined) shared[k] = db[k]; }
  fetch('/api/data/aegir_shared', {
    method: 'PUT', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(shared),
  })
  .then(r => { if (!r.ok) throw new Error(r.status); _kvReady = true; })
  .catch(e => console.warn('[KV] save error:', e.message))
  .finally(() => { _saving = false; });
}

// Load all shared data from KV (single read)
async function loadFromKV() {
  try {
    const resp = await fetch('/api/data/aegir_shared');
    if (resp.status === 404) { _kvReady = true; return true; } // no data yet, that's ok
    if (!resp.ok) throw new Error('HTTP ' + resp.status);
    const shared = await resp.json();
    for (const k of SHARED_KEYS) { if (shared[k] !== undefined) DB[k] = shared[k]; }
    saveLocalDB();
    _kvReady = true;
    render();
    return true;
...
function getSyncStatus() { return { status: _kvReady ? 'ok' : 'local' }; }
...
function renderHeader() {
  ...
  return h('header', { class: 'header' }, [
    ...
    h('div', { style: { display:'flex', gap:'12px', alignItems:'center' } }, [
      h('div', { class: 'mono', style: { fontSize:'9px', color: syncInfo.status === 'ok' ? 'var(--green)' : 'var(--amber)', cursor:'pointer', padding:'4px 8px', border:'1px solid ' + (syncInfo.status === 'ok' ? 'var(--green)' : 'var(--amber)'), borderRadius:'4px' }, onClick: () => { loadFromKV(); } }, syncInfo.status === 'ok' ? '✓ Online' : '○ Local'),
  ...
  bg.appendChild(h('div', { class: 'modal' }, [
    ...
    h('div', { class: 'modal-body' }, [
      h('div', { style: { marginBottom: '20px' } }, [
        ...
        h('button', { class: 'btn-primary', onClick: () => { loadFromKV().then(() => { showToast('Dados atualizados do servidor'); setModal(null); }); }}, '↻ Atualizar do Servidor'),
      ]),
