...
async function dbLoadAll() {
  try {
    ...
    for (const row of rows) {
      ...
      if (row.key.startsWith('fund:')) {
        funds.push(row.value);
      } else if (row.key.startsWith('carteira:')) {
        // carteira:cnpj:date
        const parts = row.key.split(':');
        const cnpj = parts[1];
        if (!carteirasXML[cnpj]) carteirasXML[cnpj] = [];
        carteirasXML[cnpj].push(row.value);
      } else if (row.key.startsWith('misc:')) {
        const mkey = row.key.slice(5);
        misc[mkey] = row.value;
      }
    }
    ...
    for (const [k, v] of Object.entries(misc)) DB[k] = v;

    saveLocalDB();
    _dbReady = true;
    render();
    console.log('[SB] rebuilt:', funds.length, 'funds,', Object.keys(carteirasXML).length, 'CNPJs');
    return true;
  } catch (e) {
