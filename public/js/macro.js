...
function renderRealEstateLens() {
  ...
  if (!series) {
    ...
    return h('div', { class: 'content fade-up' }, [
      pageHead('Macro Intelligence · Mercado Imobiliário', 'Mercado <em>Imobiliário</em>', 'Carregando dados BCB…'),
  ...
  const cycle = computeRealEstateCycle(indicators);
  const activeTab = state._re_tab || 'overview';

  // Derived RE indicators
  const reIndicators = computeREIndicators(series);

  return h('div', { class: 'content fade-up' }, [
    pageHead('Macro Intelligence · Mercado Imobiliário', 'Mercado <em>Imobiliário</em>',
      'Painel completo do mercado imobiliário brasileiro: financiamento (concessões, saldo, taxas), inadimplência, poupança/SBPE, preços. Dados oficiais BCB SGS.'),
    ...
    activeTab === 'overview' ? renderREOverview(series, indicators, cycle, reIndicators) :
    activeTab === 'concessoes' ? renderREConcessoes(series) :
    activeTab === 'saldo' ? renderRESaldo(series) :
    activeTab === 'taxas' ? renderRETaxas(series) :
    activeTab === 'inadimp' ? renderREInadimplencia(series) :
    activeTab === 'sbpe' ? renderRESBPE(series) : null,
...
function computeREIndicators(series) {
  const out = {};
  const latest = (key) => { const s = series[key]; return s?.length > 0 ? s[s.length - 1].value : null; };
  const prev = (key, n) => { const s = series[key]; return s?.length > n ? s[s.length - 1 - n].value : null; };
  const yoy = (key) => {
    const s = series[key];
    if (!s || s.length < 13) return null;
    const now = s[s.length - 1].value;
    const ago = s[s.length - 13].value;
    return ago > 0 ? ((now / ago) - 1) * 100 : null;
  };

  // Concessões YoY
  out.concPFYoY = yoy('concImobPF');
  out.concPJYoY = yoy('concImobPJ');
  // Saldo YoY
  out.saldoTotalYoY = yoy('credimob');
  out.saldoPFYoY = yoy('credimobPF');
  // Poupança
  out.poupCaptLiq = latest('poupCaptLiq');
  out.poupSaldo = latest('poupSaldo');
  out.poupSaldoYoY = yoy('poupSaldo');
  // Spread taxa imob vs SELIC
  const txPF = latest('txImobPF');
  const selic = latest('selic');
  out.spreadImobSelic = (txPF != null && selic != null) ? txPF - selic : null;
  // Juros real imob (taxa PF - IPCA 12m)
  const ipca12 = latest('ipca12m');
  out.jurosRealImob = (txPF != null && ipca12 != null) ? txPF - ipca12 : null;
  // Market share SFH vs SFI
  const merc = latest('concImobPFMerc');
  const reg = latest('concImobPFReg');
  out.shareSFI = (merc != null && reg != null && (merc + reg) > 0) ? (merc / (merc + reg)) * 100 : null;
  out.shareSFH = out.shareSFI != null ? 100 - out.shareSFI : null;

  return out;
}

function renderREOverview(series, indicators, cycle, re) {
  return h('div', {}, [
    // Cycle
    cycle && h('div', { style: { marginBottom: '28px' } }, [renderCycleCard('Fase do Ciclo Imobiliário', cycle, 'realestate')]),

    // KPI strip
    h('div', { class: 'macro-section-subhead' }, 'Termômetro do Mercado Imobiliário'),
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '24px' } }, [
      ...
      renderBigIndicator('Spread Imob-SELIC',   re.spreadImobSelic,            'p.p.',   'Taxa PF - SELIC'),
      renderBigIndicator('Juros Real Imob.',    re.jurosRealImob,              '% a.a.', 'Taxa PF - IPCA 12m'),
      renderBigIndicator('Saldo Total YoY',     re.saldoTotalYoY,              '%',      'Crescimento 12m'),
      renderBigIndicator('Concessões PF YoY',   re.concPFYoY,                  '%',      'Variação 12m'),
      renderBigIndicator('Inadimplência PF',    latestVal(series, 'inadImobPF'), '%',     'Habitacional PF'),
      renderBigIndicator('Captação Poupança',   re.poupCaptLiq,                'R$ mi',  re.poupCaptLiq > 0 ? 'Captação positiva' : 'Resgate líquido'),
      renderBigIndicator('Share SFI (mercado)', re.shareSFI,                   '%',      `SFH: ${re.shareSFH != null ? re.shareSFH.toFixed(0) : '?'}%`),
    ]),

    // Key series
    renderMacroSection('Preços & Custos de Construção', [
      { key: 'incc',    series }, { key: 'igpm',    series },
      { key: 'ivgr',    series }, { key: 'ipca12m', series },
    ], indicators, []),

    renderMacroSection('Visão Rápida — Financiamento', [
      { key: 'concImobPF', series }, { key: 'credimob', series },
      { key: 'txImobPF',   series }, { key: 'inadImobPF', series },
    ], indicators, []),
  ]);
}

function renderREConcessoes(series) {
  return h('div', {}, [
    h('div', { class: 'macro-section-subhead' }, 'Concessões de Crédito Imobiliário (fluxo mensal)'),
    h('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--text-faint)', fontStyle: 'italic' } },
      'Novas operações de financiamento contratadas no período. Regulado = SFH (taxas tabeladas). Mercado = SFI (taxas livres).'),
    renderMacroSection('Pessoa Física', [
      { key: 'concImobPF',     series }, { key: 'concImobPFMerc', series },
      { key: 'concImobPFReg',  series },
    ], {}, []),
    renderMacroSection('Pessoa Jurídica', [
      { key: 'concImobPJ',     series }, { key: 'concImobPJMerc', series },
    ], {}, []),
  ]);
}

function renderRESaldo(series) {
  return h('div', {}, [
    h('div', { class: 'macro-section-subhead' }, 'Saldo da Carteira de Crédito Imobiliário'),
    h('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--text-faint)', fontStyle: 'italic' } },
      'Estoque total de crédito direcionado ao financiamento imobiliário. Indica o tamanho e crescimento do mercado.'),
    renderMacroSection('Saldo por Segmento', [
      { key: 'credimob',   series }, { key: 'credimobPF', series },
      { key: 'credimobPJ', series },
    ], {}, []),
  ]);
}

function renderRETaxas(series) {
  return h('div', {}, [
    h('div', { class: 'macro-section-subhead' }, 'Taxas de Juros — Financiamento Imobiliário'),
    h('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--text-faint)', fontStyle: 'italic' } },
      'Taxa média ponderada das novas concessões. Regulada = SFH (juros limitados por lei). Mercado = SFI (taxas livres de mercado).'),
    renderMacroSection('Pessoa Física', [
      { key: 'txImobPF',     series }, { key: 'txImobPFMerc', series },
      { key: 'txImobPFReg',  series },
    ], {}, []),
    renderMacroSection('Pessoa Jurídica', [
      { key: 'txImobPJMerc', series },
    ], {}, []),
...
function renderREInadimplencia(series) {
  return h('div', {}, [
    h('div', { class: 'macro-section-subhead' }, 'Inadimplência — Crédito Imobiliário'),
    h('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--text-faint)', fontStyle: 'italic' } },
      'Percentual da carteira com atraso > 90 dias. Comparação com inadimplência geral do SFN.'),
    renderMacroSection('Imobiliário (Direcionado)', [
      { key: 'inadImobPF',  series }, { key: 'inadImobPJ', series },
      { key: 'inadhabit',   series },
    ], {}, []),
    renderMacroSection('Crédito Geral (referência)', [
      { key: 'inadi',       series }, { key: 'inadipj',    series },
    ], {}, []),
  ]);
}

function renderRESBPE(series) {
  return h('div', {}, [
    h('div', { class: 'macro-section-subhead' }, 'Poupança & SBPE'),
    h('div', { style: { marginBottom: '8px', fontSize: '12px', color: 'var(--text-faint)', fontStyle: 'italic' } },
      'O SBPE é a principal fonte de funding do crédito imobiliário no Brasil. Os bancos direcionam 65% dos depósitos de poupança para financiamentos habitacionais.'),
    renderMacroSection('Caderneta de Poupança', [
      { key: 'poupSaldo',   series }, { key: 'poupCaptLiq', series },
    ], {}, []),
  ]);
}

function latestVal(series, key) {
  const s = series[key];
