...
async function authSignUp(email, password, name) {
  ...
  if (data.error) throw new Error(data.error.message || data.msg || 'Erro no cadastro');
  if (data.access_token) {
    _session = data;
    localStorage.setItem('aegir_session', JSON.stringify(data));
    // Update profile name
...
async function authLogin(email, password) {
  ...
  const data = await resp.json();
  if (data.error) throw new Error(data.error_description || data.error.message || 'Login inválido');
  _session = data;
  localStorage.setItem('aegir_session', JSON.stringify(data));
  return data;
}
