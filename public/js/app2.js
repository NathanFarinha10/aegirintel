...
async function ensurePDFReady() {
  if (_pdfReady && window.jspdf?.jsPDF) return true;
  if (_pdfLoading) {
    // Wait for ongoing load
    for (let i = 0; i < 50; i++) { await new Promise(r => setTimeout(r, 100)); if (window.jspdf?.jsPDF) { _pdfReady = true; return true; } }
    return false;
  }
  _pdfLoading = true;
  try {
    ...
    _pdfLoading = false;
    showToast('Erro ao carregar biblioteca PDF', true);
    return false;
  }
}
