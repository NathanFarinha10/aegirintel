...
// Save: localStorage always + Supabase for shared keys (only changed key)
let _lastSavedKey = null;
function saveDB(db, changedKey) {
  saveLocalDB();
  if (changedKey && SHARED_KEYS.includes(changedKey)) {
    dbPut(changedKey, db[changedKey]);
  } else {
    // No specific key — save all shared keys
    for (const k of SHARED_KEYS) { if (db[k] !== undefined) dbPut(k, db[k]); }
  }
}

// Convenience: save only a specific shared key
function saveShared(key) {
  saveLocalDB();
  if (DB[key] !== undefined) dbPut(key, DB[key]);
}
