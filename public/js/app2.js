...
async function sbGetByPrefix(prefix) {
  const resp = await fetch(SB_URL + '/rest/v1/shared_data?key=like.' + encodeURIComponent(prefix + '%') + '&select=key,value', { headers: SB_HEADERS });
...
async function sbGetAll() {
  const resp = await fetch(SB_URL + '/rest/v1/shared_data?select=key,value&order=key', { headers: SB_HEADERS });
  if (!resp.ok) throw new Error('HTTP ' + resp.status);
  return resp.json();
}
...
// Save a fund (individual row: fund:{id})
function saveFundToServer(fund) {
  sbUpsert('fund:' + fund.id, fund);
}

// Delete a fund from server
function deleteFundFromServer(fundId) {
  sbDelete('fund:' + fundId);
}

// Save a carteira (individual row: carteira:{cnpj}:{date})
function saveCarteiraToServer(cnpj, carteira) {
  const date = carteira.fund?.dtposicao || 'unknown';
  sbUpsert('carteira:' + cnpj + ':' + date, carteira);
}

// Save any misc shared key (comites, anbimaMatrix, etc.)
function saveSharedKey(key, value) {
  sbUpsert('misc:' + key, value);
}

// Load everything from Supabase → rebuild DB
async function dbLoadAll() {
  try {
    const rows = await sbGetAll();
    console.log('[SB] loaded', rows.length, 'rows');

    const funds = [];
    const carteirasXML = {};
    const comites = {};
    const research = [];
    const resgateForecasts = {};
    const assetAnalyses = [];
    ...
    for (const row of rows) {
      ...
      if (k.startsWith('fund:')) {
        ...
        const cnpj = k.split(':')[1];
        if (!carteirasXML[cnpj]) carteirasXML[cnpj] = [];
        carteirasXML[cnpj].push(v);
      } else if (k.startsWith('comite:')) {
        const fundId = k.split(':')[1];
        if (!comites[fundId]) comites[fundId] = [];
        comites[fundId].push(v);
      } else if (k.startsWith('research:')) {
        research.push(v);
      } else if (k.startsWith('resgate:')) {
        const fundId = k.split(':')[1];
        if (!resgateForecasts[fundId]) resgateForecasts[fundId] = [];
        resgateForecasts[fundId].push(v);
      } else if (k.startsWith('haircut:')) {
        const fundId = k.split(':')[1];
        if (!DB.liquidityHaircuts) DB.liquidityHaircuts = {};
        DB.liquidityHaircuts[fundId] = v;
      } else if (k.startsWith('asset:')) {
        assetAnalyses.push(v);
      } else if (k.startsWith('equity:')) {
        equityPortfolio.push(v);
      } else if (k.startsWith('task:')) {
        tasks.push(v);
      } else if (k.startsWith('misc:')) {
        const mkey = k.slice(5);
        DB[mkey] = v;
      }
    }

    // Sort carteiras by date
    for (const cnpj of Object.keys(carteirasXML)) {
      carteirasXML[cnpj].sort((a, b) => (a.fund?.dtposicao || '').localeCompare(b.fund?.dtposicao || ''));
    }

    // Sort comites by date
    for (const fid of Object.keys(comites)) {
      comites[fid].sort((a, b) => (a.updated_at || '').localeCompare(b.updated_at || ''));
    }

    DB.funds = funds;
    DB.carteirasXML = carteirasXML;
    DB.comites = comites;
    ...
    _dbReady = true;
    render();
    console.log('[SB]', funds.length, 'funds,', Object.keys(carteirasXML).length, 'CNPJs,', Object.keys(comites).length, 'comitês,', research.length, 'research,', assetAnalyses.length, 'assets,', equityPortfolio.length, 'equity');
    return true;
  } catch (e) {
...
// saveDB: only saves to localStorage (shared data is pushed individually by each operation)
...
function getAnalystName() {
  return DB.settings?.analyst_name || 'Analista';
}
...
function renderMyTasks() {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    h('div', { style: { display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '20px' } }, [
      h('span', { class: 'mono', style: { fontSize: '10px', color: 'var(--text-faint)' } }, 'Meu nome:'),
      h('input', { class: 'form-field-input', style: { width: '200px', fontSize: '12px' }, value: name, id: 'wf-my-name',
        onchange: e => { if (!DB.settings) DB.settings = {}; DB.settings.analyst_name = e.target.value.trim(); saveDB(DB); render(); }
      }),
    ]),
  ...
  return h('div', { style: { borderTop: '2px solid var(--amber)', paddingTop: '20px', marginTop: '12px' } }, [
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' } }, [
      ...
      h('div', {}, [
        ...
        h('input', { class: 'form-field-input', id: 'tf-assignee', value: task.assignee || getAnalystName() }),
      ]),
      h('div', {}, [
