    ...
    NAV_SECTIONS.map(sec =>
      h('div', { class: 'sidebar-section' }, [
        ...
        h('div', { class: 'sidebar-nav' },
          sec.items.filter(it => {
            if (it.id === 'am_new' && !userCan('edit_fund')) return false;
            if (it.id === 'wf_new' && !userCan('create_task')) return false;
            return true;
          }).map(it => h('button', {
  ...
  return h('div', { class: 'content fade-up' }, [
    ...
    !selected && subTab !== 'panorama'
      ...
      : subTab === 'panorama' ? renderFundPanorama(fund, carteiras) :
        ...
        subTab === 'rentabilidade' ? renderRentabilidade(fund, carteiras) :
        subTab === 'fluxo' ? renderCarteiraFluxo(selected, fund) :
        subTab === 'pipeline' ? renderDailyPipeline(selected, fund) :
        subTab === 'provisoes' ? renderCarteiraProvisoes(selected) :
        subTab === 'imoveis' ? renderCarteiraImoveis(selected) :
        subTab === 'datas' ? renderFundDates(fund) : null,
  ]);
}
...
const BRAND_CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'Segoe UI', Arial, sans-serif; color: #1a1815; font-size: 11px; line-height: 1.6; }
  .pdf-header { background: #1a1815; color: #fff; padding: 20px 30px; display: flex; justify-content: space-between; align-items: center; }
  .pdf-header h1 { font-size: 18px; font-weight: 600; }
  .pdf-header .sub { font-size: 10px; color: #9a958c; margin-top: 2px; }
  .pdf-header .brand { text-align: right; font-size: 9px; color: #9a958c; }
  .pdf-header .brand .ae { font-size: 14px; color: #b8863c; font-weight: 700; }
  .pdf-header img { height: 36px; margin-right: 16px; }
  .pdf-body { padding: 20px 30px; }
  .pdf-section { border-left: 3px solid #b8863c; padding-left: 10px; margin: 18px 0 10px; font-size: 14px; font-weight: 700; color: #1a1815; }
  .pdf-kv { display: flex; gap: 8px; padding: 2px 0; font-size: 11px; }
  .pdf-kv .label { color: #9a958c; min-width: 130px; }
  .pdf-kv .val { font-weight: 600; }
  .pdf-text { font-size: 11px; line-height: 1.7; margin: 6px 0; white-space: pre-wrap; }
  .pdf-kpis { display: flex; gap: 12px; flex-wrap: wrap; margin: 12px 0; }
  .pdf-kpi { border: 1px solid #e0ddd8; border-radius: 6px; padding: 10px 14px; min-width: 120px; }
  .pdf-kpi .kl { font-size: 8px; text-transform: uppercase; letter-spacing: 0.1em; color: #9a958c; }
  .pdf-kpi .kv { font-size: 16px; font-weight: 700; margin-top: 2px; }
  .pdf-kpi .ks { font-size: 9px; color: #9a958c; }
  table { width: 100%; border-collapse: collapse; margin: 8px 0 16px; font-size: 10px; }
  th { background: #1a1815; color: #fff; padding: 6px 10px; text-align: left; font-size: 8px; text-transform: uppercase; letter-spacing: 0.08em; }
  th.r { text-align: right; }
  td { padding: 5px 10px; border-bottom: 1px solid #e8e5e0; }
  td.r { text-align: right; font-family: 'Courier New', monospace; }
  td.b { font-weight: 600; }
  tr:nth-child(even) { background: #f8f6f3; }
  .pdf-footer { margin-top: 30px; padding-top: 10px; border-top: 1px solid #ccc; font-size: 8px; color: #999; display: flex; justify-content: space-between; }
...
function openPrintReport(title, subtitle, bodyHTML) {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>' + title + '</title><style>' + BRAND_CSS + '</style></head><body>' +
    '<div class="pdf-header">' +
      '<div style="display:flex;align-items:center">' + logoImg +
        '<div><h1>' + title + '</h1><div class="sub">' + (subtitle || '') + '</div></div>' +
      '</div>' +
      '<div class="brand"><div class="ae">Æ</div>Aegir·Intel<br>' + new Date().toLocaleDateString('pt-BR') + '</div>' +
    '</div>' +
    '<div class="pdf-body">' + bodyHTML + '</div>' +
    '<div class="pdf-footer"><span>Confidencial — Gerado pelo Aegir·Intel</span><span>' + new Date().toLocaleDateString('pt-BR') + '</span></div>' +
...
// Helpers
function pSec(t) { return '<div class="pdf-section">' + t + '</div>'; }
...
function pKPI(label, value, sub) { return '<div class="pdf-kpi"><div class="kl">' + label + '</div><div class="kv">' + value + '</div>' + (sub ? '<div class="ks">' + sub + '</div>' : '') + '</div>'; }
...
async function exportCarteiraPDF(parsed, fund) {
  ...
  let b = '<div class="pdf-kpis">' + pKPI('PL', formatBRL(f.patliq)) + pKPI('Cota', f.valorcota ? 'R$ ' + f.valorcota.toFixed(6) : '—') + pKPI('Caixa', formatBRL(stats.totalCaixa), (stats.totalCaixa/pl*100).toFixed(1) + '% PL') + pKPI('Posições', stats.totalAtivos) + '</div>';
...
async function exportFundOverviewPDF(fund) {
  const carteiras = getCarteirasXML(fund.cnpj);
  const latest = carteiras.length > 0 ? carteiras[carteiras.length-1] : null;
  let b = pKV('Classificação', fund.classification) + pKV('Estratégia', fund.strategy) + pKV('Benchmark', fund.benchmark) + pKV('Carteiras', carteiras.length);
  if (latest) b += pKV('Última Carteira', latest.fund.dtposicao) + pKV('PL', formatBRL(latest.fund.patliq)) + pKV('Cota', latest.fund.valorcota ? 'R$ ' + latest.fund.valorcota.toFixed(6) : '—');
...
async function exportRiskReportPDF(fund, risk, carteira) {
  if (!risk) { showToast('Sem dados de risco', true); return; }
  let b = '<div class="pdf-kpis">' + pKPI('PL', formatBRL(risk.pl)) + pKPI('Tipo', risk.fundProfile?.label||'—') + pKPI('HHI', (risk.concentration.hhi*10000).toFixed(0)) + pKPI('Top 5', risk.concentration.top5Pct.toFixed(1)+'%') + '</div>';
  b += pSec('Matriz de Risco');
  const rows = [['Concentração', risk.concentration.hhi<0.15?'Baixo':risk.concentration.hhi<0.25?'Médio':'Alto', 'HHI: '+(risk.concentration.hhi*10000).toFixed(0)], ['Liquidez', risk.liquidity.cashRatio>5?'Baixo':risk.liquidity.cashRatio>1?'Médio':'Alto', risk.liquidity.cashRatio.toFixed(2)+'% PL']];
...
async function initPDFAsync(title, orientation) {
  if (!window.jspdf?.jsPDF) {
    showToast('Biblioteca PDF não carregou. Recarregue a página.', true);
    return null;
  }
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation: orientation || 'portrait', unit: 'mm', format: 'a4' });
  doc._aegir = { pageW: orientation === 'landscape' ? 297 : 210, pageH: orientation === 'landscape' ? 210 : 297, margin: 20, y: 20 };
  return doc;
}
...
function pdfHeader(doc, title, subtitle) {
  ...
  doc.rect(0, 0, a.pageW, 32, 'F');

  // Logo
  if (logo) {
    try { doc.addImage(logo, 'PNG', a.margin, 6, 20, 20); } catch(e) {}
  }
  const logoOffset = logo ? 46 : a.margin;

  // Title
  doc.setTextColor(...BRAND.white);
  doc.setFontSize(16); doc.setFont('helvetica', 'bold');
  doc.text(title, logoOffset, 15);
  doc.setFontSize(9); doc.setFont('helvetica', 'normal');
  doc.setTextColor(...BRAND.gray);
  doc.text(subtitle || '', logoOffset, 22);

  // Aegir branding
  doc.setFontSize(8); doc.setTextColor(100, 100, 100);
  doc.text('Aegir·Intel', a.pageW - a.margin, 12, { align: 'right' });
  doc.text(new Date().toLocaleDateString('pt-BR'), a.pageW - a.margin, 18, { align: 'right' });
...
function pdfFooter(doc, pageNum) {
  const a = doc._aegir;
  doc.setDrawColor(200); doc.line(a.margin, a.pageH - 15, a.pageW - a.margin, a.pageH - 15);
  doc.setFontSize(7); doc.setTextColor(150);
  doc.text('Confidencial — Gerado pelo Aegir·Intel', a.margin, a.pageH - 10);
  doc.text('Página ' + pageNum, a.pageW - a.margin, a.pageH - 10, { align: 'right' });
}

function pdfSection(doc, title) {
  const a = doc._aegir;
  if (a.y > a.pageH - 40) { doc.addPage(); a.y = 20; }
  doc.setFillColor(...BRAND.primary);
  doc.rect(a.margin, a.y, 3, 6, 'F');
  doc.setTextColor(...BRAND.dark);
  doc.setFontSize(12); doc.setFont('helvetica', 'bold');
  doc.text(title, a.margin + 7, a.y + 5);
  a.y += 12;
...
function pdfText(doc, text, opts) {
  ...
  const lines = doc.splitTextToSize(text, a.pageW - 2 * a.margin);
  for (const line of lines) {
    if (a.y > a.pageH - 20) { doc.addPage(); a.y = 20; }
    doc.text(line, a.margin, a.y);
    a.y += (opts?.lineH || 4.5);
  }
  doc.setFont('helvetica', 'normal');
  a.y += 2;
...
function pdfKV(doc, label, value) {
  ...
  doc.text(label + ':', a.margin, a.y);
  doc.setTextColor(...BRAND.dark); doc.setFont('helvetica', 'bold');
  doc.text(String(value || '—'), a.margin + 45, a.y);
  doc.setFont('helvetica', 'normal');
  a.y += 5;
}
...
function pdfTable(doc, headers, rows, opts) {
  ...
  doc.autoTable({
    ...
    head: [headers],
    body: rows,
    margin: { left: a.margin, right: a.margin },
    styles: { fontSize: 8, cellPadding: 2.5, textColor: BRAND.dark, lineColor: [220,220,220] },
    headStyles: { fillColor: BRAND.dark, textColor: BRAND.white, fontStyle: 'bold', fontSize: 7 },
    alternateRowStyles: { fillColor: [248, 246, 243] },
    ...(opts || {}),
  });
  a.y = doc.lastAutoTable.finalY + 8;
}

function pdfSave(doc, filename) {
  const pages = doc.getNumberOfPages();
  for (let i = 1; i <= pages; i++) { doc.setPage(i); pdfFooter(doc, i); }
  doc.save(filename);
...
// Thesis Report
async function exportTesePDF(tese, fund) {
  const doc = await initPDFAsync('Tese de Investimento'); if (!doc) return;
  const st = THESIS_STATUS[tese.status] || {};
  const conv = CONVICTION_LEVELS[tese.conviction] || {};

  pdfHeader(doc, 'Tese: ' + tese.title, fund?.name || '');
  pdfKV(doc, 'Fundo', fund?.name);
  pdfKV(doc, 'Status', st.label || tese.status);
  pdfKV(doc, 'Convicção', conv.label || tese.conviction);
  pdfKV(doc, 'Horizonte', tese.horizon || '—');
  doc._aegir.y += 4;

  pdfSection(doc, 'Narrativa');
  pdfText(doc, tese.narrative || 'Sem narrativa');

  if (tese.target_assets?.length > 0) {
    pdfSection(doc, 'Ativos-Alvo');
    pdfTable(doc, ['Ticker', 'Nome', 'Papel', 'Peso (%)'],
      tese.target_assets.map(a => [a.ticker, a.name, a.role, a.target_weight || '—']));
  }

  pdfSection(doc, 'Cenários');
  pdfTable(doc, ['Cenário', 'Probabilidade', 'Retorno Esperado', 'Descrição'], [
    ['Base', (tese.base_case?.probability || '—') + '%', tese.base_case?.expected_return || '—', tese.base_case?.description || '—'],
    ['Bull', (tese.bull_case?.probability || '—') + '%', tese.bull_case?.expected_return || '—', tese.bull_case?.description || '—'],
    ['Bear', (tese.bear_case?.probability || '—') + '%', tese.bear_case?.expected_return || '—', tese.bear_case?.description || '—'],
  ]);
  ...
  if (tese.revision_triggers?.length > 0) {
    pdfSection(doc, 'Triggers de Revisão');
    tese.revision_triggers.forEach(t => pdfText(doc, '• ' + t));
  }

  pdfSave(doc, 'tese-' + (tese.title || 'report').replace(/\s+/g, '-').substring(0, 40) + '.pdf');
}

// Committee Minutes Report
async function exportAtaPDF(comite, fund) {
  const doc = await initPDFAsync('Ata de Comitê'); if (!doc) return;

  pdfHeader(doc, 'Ata de Comitê de Investimentos', fund?.name || '');
  pdfKV(doc, 'Fundo', fund?.name);
  pdfKV(doc, 'Data', comite.date || '—');
  pdfKV(doc, 'Tipo', comite.type || '—');
  pdfKV(doc, 'Participantes', (comite.participants || []).join(', '));
  doc._aegir.y += 4;

  pdfSection(doc, 'Pauta');
  pdfText(doc, comite.agenda || 'Sem pauta');

  if (comite.proposals?.length > 0) {
    pdfSection(doc, 'Propostas');
    comite.proposals.forEach((p, i) => {
      pdfText(doc, (i + 1) + '. ' + p.description, { bold: true });
      pdfKV(doc, '  Tipo', p.type); pdfKV(doc, '  Valor', p.value || '—');
      pdfKV(doc, '  Decisão', p.decision || 'Pendente');
      doc._aegir.y += 2;
    });
  }

  pdfSection(doc, 'Deliberações');
  pdfText(doc, comite.deliberations || 'Sem deliberações registradas');

  if (comite.gestorParecer) {
    pdfSection(doc, 'Parecer do Gestor');
    pdfText(doc, comite.gestorParecer);
  }

  pdfSave(doc, 'ata-comite-' + (comite.date || 'report') + '.pdf');
}

// Carteira Report
async function exportCarteiraPDF(parsed, fund) {
  const doc = await initPDFAsync('Relatório de Carteira', 'landscape'); if (!doc) return;
  const f = parsed.fund;
  const stats = parsed.stats || {};
  const pos = parsed.positions || {};

  pdfHeader(doc, 'Carteira — ' + (fund?.name || f.cnpj), f.dtposicao);
  pdfKV(doc, 'CNPJ', f.cnpj); pdfKV(doc, 'Data', f.dtposicao);
  pdfKV(doc, 'PL', formatBRL(f.patliq)); pdfKV(doc, 'Cota', f.valorcota ? 'R$ ' + f.valorcota.toFixed(6) : '—');
  pdfKV(doc, 'Caixa', formatBRL(stats.totalCaixa)); pdfKV(doc, 'Posições', String(stats.totalAtivos || 0));
  doc._aegir.y += 4;

  if ((pos.acoes || []).length > 0) {
    pdfSection(doc, 'Ações / FIIs (' + pos.acoes.length + ')');
    pdfTable(doc, ['Ticker', 'Qtd', 'PU (R$)', 'Valor (R$)', '% PL', 'PDD %'],
      pos.acoes.sort((a, b) => (b.valorfindisp || 0) - (a.valorfindisp || 0)).map(p => [
        p.codativo, (p.qtdisponivel || 0).toLocaleString('pt-BR'), (p.puposicao || 0).toFixed(2),
        formatBRL(p.valorfindisp), ((p.valorfindisp || 0) / (f.patliq || 1) * 100).toFixed(2) + '%',
        (p.percprovcred || 0).toFixed(1) + '%',
      ]));
  }

  if ((pos.titprivado || []).length > 0) {
    pdfSection(doc, 'Títulos Privados (' + pos.titprivado.length + ')');
    pdfTable(doc, ['Código', 'Indexador', 'Cupom', 'Vencimento', 'Valor (R$)', '% PL', 'PDD', 'Risco'],
      pos.titprivado.sort((a, b) => (b.valorfindisp || 0) - (a.valorfindisp || 0)).map(p => [
        p.codativo, p.indexador || '—', p.coupom ? p.coupom.toFixed(2) + '%' : '—',
        p.dtvencimento || '—', formatBRL(p.valorfindisp),
        ((p.valorfindisp || 0) / (f.patliq || 1) * 100).toFixed(2) + '%',
        (p.percprovcred || 0).toFixed(1) + '%', p.nivelrsc || '—',
      ]));
  }

  if ((pos.cotas || []).length > 0) {
    pdfSection(doc, 'Cotas de Fundos (' + pos.cotas.length + ')');
    pdfTable(doc, ['CNPJ Fundo', 'Qtd', 'PU', 'Valor (R$)', '% PL'],
      pos.cotas.map(p => {
        const val = p.valorfindisp || ((p.qtdisponivel || 0) * (p.puposicao || 0));
        return [p.cnpjfundo || '—', (p.qtdisponivel || 0).toLocaleString('pt-BR'),
          'R$ ' + (p.puposicao || 0).toFixed(6), formatBRL(val), (val / (f.patliq || 1) * 100).toFixed(2) + '%'];
      }));
  }

  pdfSave(doc, 'carteira-' + (fund?.name || f.cnpj).replace(/\s+/g, '-').substring(0, 30) + '-' + f.dtposicao + '.pdf');
}

// Fund Overview Report
async function exportFundOverviewPDF(fund) {
  const doc = await initPDFAsync('Relatório do Fundo'); if (!doc) return;
  const carteiras = getCarteirasXML(fund.cnpj);
  const latest = carteiras.length > 0 ? carteiras[carteiras.length - 1] : null;

  pdfHeader(doc, fund.name, fund.cnpj + ' · ' + (fund.classification || '') + ' · ' + (fund.strategy || ''));
  pdfKV(doc, 'Classificação', fund.classification);
  pdfKV(doc, 'Estratégia', fund.strategy);
  pdfKV(doc, 'Benchmark', fund.benchmark);
  pdfKV(doc, 'Carteiras', String(carteiras.length));
  if (latest) { pdfKV(doc, 'Última Carteira', latest.fund.dtposicao); pdfKV(doc, 'PL', formatBRL(latest.fund.patliq)); pdfKV(doc, 'Cota', latest.fund.valorcota ? 'R$ ' + latest.fund.valorcota.toFixed(6) : '—'); }
  doc._aegir.y += 4;

  // Rentabilidade
  if (carteiras.length >= 2) {
    pdfSection(doc, 'Rentabilidade');
    const sorted = [...carteiras].sort((a, b) => (a.fund.dtposicao || '').localeCompare(b.fund.dtposicao || ''));
    const lc = sorted[sorted.length - 1].fund.valorcota;
    const periods = [
      ['1 Mês', 30], ['3 Meses', 90], ['6 Meses', 180], ['12 Meses', 365], ['Desde Início', 9999],
    ];
    const retRows = [];
    for (const [label, days] of periods) {
      const ref = days === 9999 ? sorted[0] : (() => { const t = new Date(new Date(sorted[sorted.length-1].fund.dtposicao).getTime() - days * 86400000); let best = null, bd = Infinity; for (const c of sorted) { if (!c.fund.valorcota) continue; const d = Math.abs(new Date(c.fund.dtposicao) - t); if (d < bd) { bd = d; best = c; } } return best; })();
      if (ref?.fund?.valorcota && lc) {
        const ret = ((lc - ref.fund.valorcota) / ref.fund.valorcota) * 100;
        retRows.push([label, ref.fund.dtposicao, (ret >= 0 ? '+' : '') + ret.toFixed(2) + '%']);
      }
    }
    if (retRows.length > 0) pdfTable(doc, ['Período', 'Referência', 'Retorno'], retRows);
  }

  // Thesis
  if (fund.thesis_summary) { pdfSection(doc, 'Tese de Investimento'); pdfText(doc, fund.thesis_summary); }

  pdfSave(doc, 'relatorio-' + fund.name.replace(/\s+/g, '-').substring(0, 30) + '.pdf');
}

// === Risk Report PDF ===
async function exportRiskReportPDF(fund, risk, carteira) {
  if (!risk) { showToast('Sem dados de risco', true); return; }
  const doc = await initPDFAsync('Relatório de Risco', 'portrait'); if (!doc) return;
  const fp = risk.fundProfile || {};

  pdfHeader(doc, 'Relatório de Risco', fund.name + ' · ' + risk.fund.dtposicao);
  pdfKV(doc, 'Fundo', fund.name);
  pdfKV(doc, 'PL', formatBRL(risk.pl));
  pdfKV(doc, 'Tipo', fp.label || '—');
  pdfKV(doc, 'Carteira', risk.fund.dtposicao);
  doc._aegir.y += 4;

  // Risk Matrix
  pdfSection(doc, 'Matriz de Risco');
  const riskRows = [
    ['Concentração', risk.concentration.hhi < 0.15 ? 'Baixo' : risk.concentration.hhi < 0.25 ? 'Médio' : 'Alto', 'HHI: ' + (risk.concentration.hhi * 10000).toFixed(0) + ' · Top5: ' + risk.concentration.top5Pct.toFixed(1) + '%'],
    ['Liquidez', risk.liquidity.cashRatio > 5 ? 'Baixo' : risk.liquidity.cashRatio > 1 ? 'Médio' : 'Alto', risk.liquidity.cashRatio.toFixed(2) + '% PL'],
  ];
  if (fp.hasCredit) {
    riskRows.push(['Crédito (PDD)', risk.credit.pddCoveragePct < 5 ? 'Baixo' : risk.credit.pddCoveragePct < 15 ? 'Médio' : 'Alto', 'PDD: ' + risk.credit.pddCoveragePct.toFixed(2) + '% · LGD: ' + risk.credit.portfolioLGD.toFixed(1) + '%']);
    riskRows.push(['Perda Esperada', risk.credit.expectedLoss < 1 ? 'Baixo' : risk.credit.expectedLoss < 5 ? 'Médio' : 'Alto', 'EL: ' + risk.credit.expectedLoss.toFixed(3) + '%']);
  }
  if (risk.duration.avgDuration > 0) riskRows.push(['Duration', risk.duration.avgDuration < 3 ? 'Baixo' : risk.duration.avgDuration < 7 ? 'Médio' : 'Alto', risk.duration.avgDuration.toFixed(2) + ' anos']);
  if (risk.var.var95 > 0) riskRows.push(['VaR 95% (21d)', (risk.var.var95/risk.pl*100) < 3 ? 'Baixo' : 'Médio/Alto', formatBRL(risk.var.var95) + ' (' + (risk.var.var95/risk.pl*100).toFixed(2) + '% PL)']);
  if (risk.realEstateRisk) riskRows.push(['Imobiliário', risk.realEstateRisk.inadimPct < 5 ? 'Baixo' : 'Médio/Alto', 'Cap rate: ' + risk.realEstateRisk.capRate.toFixed(2) + '% · Inadim: ' + risk.realEstateRisk.inadimPct.toFixed(1) + '%']);
  pdfTable(doc, ['Categoria', 'Nível', 'Detalhe'], riskRows);

  // Top positions
  pdfSection(doc, 'Top 5 Posições');
  pdfTable(doc, ['Posição', '% PL', 'Valor'], risk.concentration.top5.map(p => [p.name, p.pct.toFixed(2) + '%', formatBRL(p.value)]));

  // Stress
  if (risk.stress?.length > 0) {
    pdfSection(doc, 'Stress Tests');
    pdfTable(doc, ['Cenário', 'Impacto R$', 'Impacto %'], risk.stress.map(s => [s.name, formatBRL(s.impactValue), s.impactPct.toFixed(2) + '%']));
  }
  ...
  if (carteira) {
    ...
    const breaches = checks.filter(c => c.status === 'breach');
    const warnings = checks.filter(c => c.status === 'warning');
    pdfSection(doc, 'Enquadramento CVM 175');
    pdfText(doc, (breaches.length > 0 ? 'DESENQUADRADO' : warnings.length > 0 ? 'ATENÇÃO' : 'ENQUADRADO') + ' — ' + breaches.length + ' violação(ões), ' + warnings.length + ' alerta(s)', { bold: true });
    if (breaches.length > 0) pdfTable(doc, ['Regra', 'Limite', 'Atual', 'Ref.'], breaches.map(c => [c.rule, c.limit, c.actual, c.ref]));
    if (warnings.length > 0) pdfTable(doc, ['Regra', 'Limite', 'Atual', 'Ref.'], warnings.map(c => [c.rule, c.limit, c.actual, c.ref]));
  }

  pdfSave(doc, 'risco-' + fund.name.replace(/\s+/g, '-').substring(0, 30) + '.pdf');
}

// === Asset Analysis PDF ===
async function exportAssetAnalysisPDF(a) {
  if (!a) return;
  const doc = await initPDFAsync('Análise de Ativo'); if (!doc) return;
  const TIPOS = { cri: 'CRI', cra: 'CRA', debenture: 'Debênture', fii: 'FII', fidc: 'FIDC', lci_lca: 'LCI/LCA', custom: 'Outro' };

  pdfHeader(doc, 'Análise: ' + (a.name || a.ticker || '—'), (TIPOS[a.type] || a.type) + ' · ' + (a.updated_at || '').split('T')[0]);
  pdfKV(doc, 'Tipo', TIPOS[a.type] || a.type);
  pdfKV(doc, 'Ticker/Código', a.ticker || '—');
  pdfKV(doc, 'Emissor', a.issuer || '—');
  pdfKV(doc, 'Rating', a.rating || '—');
  pdfKV(doc, 'Indexador', a.indexer || '—');
  pdfKV(doc, 'Spread', a.spread ? a.spread + '%' : '—');
  pdfKV(doc, 'Vencimento', a.maturity || '—');
  pdfKV(doc, 'Status', a.status || '—');
  doc._aegir.y += 4;

  if (a.thesis) { pdfSection(doc, 'Tese / Parecer'); pdfText(doc, a.thesis); }
  if (a.risks) { pdfSection(doc, 'Riscos Identificados'); pdfText(doc, a.risks); }
  if (a.guarantees) { pdfSection(doc, 'Garantias'); pdfText(doc, a.guarantees); }

  pdfSave(doc, 'analise-' + (a.name || a.ticker || 'ativo').replace(/\s+/g, '-').substring(0, 30) + '.pdf');
}
