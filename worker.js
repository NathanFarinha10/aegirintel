...
const ALLOWED_SOURCES = {
  ...
  bcb_imob: {
    ...
    // Discovery mode: ?source=bcb_imob&discover=1&top=100 → returns distinct Info values
    // Data mode: ?source=bcb_imob&series=nome_da_serie&top=120
    buildUrl: (params) => {
      const top = params.top || '120';
      if (params.discover) {
        // Discovery or bulk fetch mode
        const skip = params.skip || '0';
        const filter = params.filter_br === '1'
          ? `&$filter=endswith(Info,'_br')`
          : '';
        return `https://olinda.bcb.gov.br/olinda/servico/MercadoImobiliario/versao/v1/odata/mercadoimobiliario?$format=json&$top=${top}&$skip=${skip}&$select=Data,Info,Valor&$orderby=Data desc${filter}`;
      }
      const series = params.series;
      if (!series) throw new Error('Missing series parameter. Use discover=1 to list available series.');
      return `https://olinda.bcb.gov.br/olinda/servico/MercadoImobiliario/versao/v1/odata/mercadoimobiliario?$format=json&$filter=Info eq '${series}'&$orderby=Data asc&$top=${top}&$select=Data,Info,Valor`;
    },
    headers: {
      'Accept': 'application/json',
      'User-Agent': 'AegirIntel/1.0',
...
export default {
  async fetch(request, env) {
    ...
    // Let Cloudflare internal paths pass through (Access, challenges, analytics)
    // These are handled by Cloudflare infrastructure before reaching the Worker,
    // but if they somehow arrive here, return 404 to avoid loops
