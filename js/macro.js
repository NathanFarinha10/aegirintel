...
async function fetchBCBSeries(code, lastN = 24) {
  // Try Netlify Function proxy first (no CORS issues), fallback to direct+proxy
  const proxyUrl = `/.netlify/functions/api-proxy?source=bcb&code=${code}&last=${lastN}`;
  ...
  for (const url of urls) {
    try {
      ...
      if (!Array.isArray(data)) {
        // Netlify proxy might wrap in { error } if upstream failed
        if (data.error) { console.warn(`BCB ${code} proxy:`, data.error); continue; }
        continue;
      }
      return data.map(row => ({
        date: row.data,
      ...
      console.warn(`BCB ${code} via ${url.substring(0, 50)}:`, err.message);
    }
  }
...
async function fetchEconomicCalendar() {
  ...
  const proxyUrl = `/.netlify/functions/api-proxy?source=finnhub_calendar&token=${token}&from=${from}&to=${to}`;
...
async function fetchFREDSeries(seriesId, apiKey, limit = 60) {
  ...
  const proxyUrl = `/.netlify/functions/api-proxy?source=fred&series_id=${seriesId}&api_key=${apiKey}&limit=${limit}&observation_start=${startStr}`;
