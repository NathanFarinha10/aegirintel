...
const BCB_SERIES = {
  ...
  ivgr:      { code: 21340, name: 'IVG-R',                group: 'realestate', unit: '% mensal', decimals: 2, name2: 'Valor Garantias Residenciais' },
  credimob:  { code: 20714, name: 'Saldo Crédito Imobiliário', group: 'realestate', unit: 'R$ mi', decimals: 0 },
  credimobPF:{ code: 20720, name: 'Crédito Imob. PF',     group: 'realestate', unit: 'R$ mi', decimals: 0 },
  // === FX ===
  ...
  inadhabit: { code: 21146, name: 'Inadimplência Habitacional', group: 'credit', unit: '%',   decimals: 2 },
...
async function fetchBCBSeries(code, lastN = 24) {
  const url = `${BCB_SGS_BASE}.${code}/dados/ultimos/${lastN}?formato=json`;
  try {
    const data = await fetchWithProxyFallback(url);
    if (!Array.isArray(data)) throw new Error('Resposta inválida do BCB');
    return data.map(row => ({
      date: row.data,
      value: parseFloat(String(row.valor).replace(',', '.')),
    }));
  } catch (err) {
    console.warn(`BCB série ${code} falhou:`, err.message);
    return null;
  }
}
...
function renderRealEstateLens() {
  ...
  if (!series) {
    ...
    return h('div', { class: 'content fade-up' }, [
      pageHead('Macro Intelligence · Real Estate Lens', 'Real Estate <em>Lens</em>', 'Carregando dados macro…'),
  ...
  return h('div', { class: 'content fade-up' }, [
    pageHead('Macro Intelligence · Real Estate Lens', 'Real Estate <em>Lens</em>',
      'Visão macroeconômica focada em Real Estate. Cruzamentos entre custos de construção, condições de financiamento, demanda e inadimplência.'),
    ...
    h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '24px' } }, [
      ...
      renderBigIndicator('Spread Construção',   indicators.spreadConstrucao,   'p.p.',    'INCC 12m - IPCA 12m'),
      renderBigIndicator('Crédito Imob. YoY',   indicators.credimobYoY,        '%',       'Variação saldo 12m'),
    ]),
    ...
    renderMacroSection('Financiamento Imobiliário', [
      { key: 'selic',     series },
      { key: 'credimob',  series },
      { key: 'credimobPF', series },
      { key: 'inadhabit', series },
    ], indicators, []),

    renderMacroSection('Demanda & Atividade', [
      { key: 'ibcbr',     series },
      { key: 'icc',       series },
    ], indicators, []),
...
function renderEventCalendar() {
  ...
  return h('div', { class: 'content fade-up' }, [
    pageHead('Macro & Rates · Event Calendar', 'Event <em>Calendar</em>',
      'Eventos macro (COPOM, FOMC, publicações de índices), earnings, dividendos de FIIs, vencimentos de dívida. Adicione manualmente o que é relevante para seu acompanhamento.'),
    ...
    h('div', { class: 'event-cal-wrap' }, [
      ...
      events.length === 0
        ? h('div', { style: { padding: '40px', textAlign: 'center', color: 'var(--text-muted)' } }, 'Nenhum evento cadastrado. Adicione eventos relevantes acima.')
...
async function fetchFREDSeries(seriesId, apiKey, limit = 60) {
  // Calculate observation_start as ~5 years ago for deep history, or limit by count
  const startDate = new Date();
  startDate.setFullYear(startDate.getFullYear() - 3);
  const startStr = startDate.toISOString().split('T')[0];

  const url = `${FRED_API_BASE}?series_id=${seriesId}&api_key=${apiKey}&file_type=json&sort_order=desc&limit=${limit}&observation_start=${startStr}`;

  try {
    const res = await fetchWithTimeout(url, 12000);
    if (!res.ok) {
      const text = await res.text();
      throw new Error(`FRED ${res.status}: ${text.substring(0, 100)}`);
    }
    const data = await res.json();
    if (!data.observations || !Array.isArray(data.observations)) {
      throw new Error('FRED: formato inesperado');
    }
    // FRED returns desc order; reverse to chronological, filter out "." (missing)
